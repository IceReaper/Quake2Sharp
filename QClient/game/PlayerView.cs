...
public class PlayerView
{
	...
	public static float SV_CalcRoll(float[] angles, float[] velocity)
	{
		...
		side = Math3D.DotProduct(velocity, PlayerView.right);
		sign = side < 0 ? -1 : 1;
		side = Math.Abs(side);
	...
	public static void SV_CalcGunOffset(edict_t ent)
	{
		...
		ent.client.ps.gunangles[Defines.ROLL] = PlayerView.xyspeed * PlayerView.bobfracsin * 0.005f;
		ent.client.ps.gunangles[Defines.YAW] = PlayerView.xyspeed * PlayerView.bobfracsin * 0.01f;
		...
		ent.client.ps.gunangles[Defines.PITCH] = PlayerView.xyspeed * PlayerView.bobfracsin * 0.005f;
		...
		for (i = 0; i < 3; i++)
		{
			ent.client.ps.gunoffset[i] += PlayerView.forward[i] * GameBase.gun_y.value;
			ent.client.ps.gunoffset[i] += PlayerView.right[i] * GameBase.gun_x.value;
			ent.client.ps.gunoffset[i] += PlayerView.up[i] * -GameBase.gun_z.value;
		}
	}
	...
	public static void SV_CalcBlend(edict_t ent)
	{
		...
		contents = GameBase.gi.pointcontents(vieworg);
		...
		if (ent.client.quad_framenum > GameBase.level.framenum)
		{
			...
			if (remaining > 30 || (remaining & 4) != 0)
				PlayerView.SV_AddBlend(0, 0, 1, 0.08f, ent.client.ps.blend);
		}
		else if (ent.client.invincible_framenum > GameBase.level.framenum)
		{
			...
			if (remaining > 30 || (remaining & 4) != 0)
				PlayerView.SV_AddBlend(1, 1, 0, 0.08f, ent.client.ps.blend);
		}
		else if (ent.client.enviro_framenum > GameBase.level.framenum)
		{
			...
			if (remaining > 30 || (remaining & 4) != 0)
				PlayerView.SV_AddBlend(0, 1, 0, 0.08f, ent.client.ps.blend);
		}
		else if (ent.client.breather_framenum > GameBase.level.framenum)
		{
			...
			if (remaining > 30 || (remaining & 4) != 0)
				PlayerView.SV_AddBlend(0.4f, 1, 0.4f, 0.04f, ent.client.ps.blend);
		}

		// add for damage
		if (ent.client.damage_alpha > 0)
		{
			PlayerView.SV_AddBlend(
	...
	public static void G_SetClientEvent(edict_t ent)
	{
		...
		if (ent.groundentity != null && PlayerView.xyspeed > 225)
		{
			if ((int)(PlayerView.current_client.bobtime + PlayerView.bobmove) != PlayerView.bobcycle)
				ent.s.@event = Defines.EV_FOOTSTEP;
		}
	}
