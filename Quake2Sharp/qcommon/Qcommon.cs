	 ...
	 */
		public static void Init(string[] args)
		{
			try
			{
				...
				var s = Com.sprintf("%4.2f %s %s %s", new Vargs(4).add(Globals.VERSION).add(Qcommon.CPUSTRING).add(Globals.__DATE__).add(Qcommon.BUILDSTRING));
		...
		public static void FrameUpdate(int msec)
		{
			try
			{
				...
				if (Globals.host_speeds.value != 0.0f)
				{
					...
					Com.Printf("update:%3i gm:%3i rf:%3i\n", new Vargs(3).add(sv).add(gm).add(rf));
				}
			}
			catch (Exception e)
			{
		...
		public static void FrameRender(int msec)
		{
			try
			{
				...
				if (Globals.host_speeds.value != 0.0f)
				{
					...
					Com.Printf("render:%3i gm:%3i rf:%3i\n", new Vargs(3).add(cl).add(gm).add(rf));
				}
			}
			catch (Exception e)
			{
