...
namespace Quake2Sharp.qcommon
{
	...
	public class CM
	{
		...
		public static qfiles.dvis_t map_vis = new(new BinaryReader(new MemoryStream(CM.map_visibility)));
     ...
     */
		public static cmodel_t CM_LoadMap(string name, bool clientload, int[] checksum)
		{
			...
			header = new qfiles.dheader_t(bbuf);
		...
		public static void CMod_LoadSubmodels(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@in = new qfiles.dmodel_t(new BinaryReader(new MemoryStream(CM.cmod_base, i * qfiles.dmodel_t.SIZE + l.fileofs, qfiles.dmodel_t.SIZE)));
		...
		public static void CMod_LoadSurfaces(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@out = CM.map_surfaces[i] = new mapsurface_t();
				@in = new texinfo_t(CM.cmod_base, l.fileofs + i * texinfo_t.SIZE, texinfo_t.SIZE);
		...
		public static void CMod_LoadNodes(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@in = new qfiles.dnode_t(new BinaryReader(new MemoryStream(CM.cmod_base, qfiles.dnode_t.SIZE * i + l.fileofs, qfiles.dnode_t.SIZE)));
				@out = CM.map_nodes[i];
		...
		public static void CMod_LoadBrushes(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@in = new qfiles.dbrush_t(new BinaryReader(new MemoryStream(CM.cmod_base, i * qfiles.dbrush_t.SIZE + l.fileofs, qfiles.dbrush_t.SIZE)));
		...
		public static void CMod_LoadLeafs(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@in = new qfiles.dleaf_t(CM.cmod_base, i * qfiles.dleaf_t.SIZE + l.fileofs, qfiles.dleaf_t.SIZE);
		...
		public static void CMod_LoadPlanes(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@in = new qfiles.dplane_t(new BinaryReader(new MemoryStream(CM.cmod_base, i * qfiles.dplane_t.SIZE + l.fileofs, qfiles.dplane_t.SIZE)));
		...
		public static void CMod_LoadBrushSides(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@in = new qfiles.dbrushside_t(
					new BinaryReader(new MemoryStream(CM.cmod_base, i * qfiles.dbrushside_t.SIZE + l.fileofs, qfiles.dbrushside_t.SIZE))
				);
		...
		public static void CMod_LoadAreas(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@in = new qfiles.darea_t(new BinaryReader(new MemoryStream(CM.cmod_base, i * qfiles.darea_t.SIZE + l.fileofs, qfiles.darea_t.SIZE)));
				@out = CM.map_areas[i];
		...
		public static void CMod_LoadAreaPortals(lump_t l)
		{
			...
			for (i = 0; i < count; i++)
			{
				@in = new qfiles.dareaportal_t(
					new BinaryReader(new MemoryStream(CM.cmod_base, i * qfiles.dareaportal_t.SIZE + l.fileofs, qfiles.dareaportal_t.SIZE))
				);
		...
		public static void CMod_LoadVisibility(lump_t l)
		{
			...
			CM.map_vis = new qfiles.dvis_t(bb);
		}
		...
		public static void CMod_LoadEntityString(lump_t l)
		{
			...
			Com.dprintln(
				"entitystring=" + CM.map_entitystring.Length + " bytes, [" + CM.map_entitystring.Substring(0, Math.Min(CM.map_entitystring.Length, 15)) + "...]"
			);
		}
     ...
     */
		public static void CM_InitBoxHull()
		{
			...
			{
				CM.map_planes[CM.numplanes], CM.map_planes[CM.numplanes + 1], CM.map_planes[CM.numplanes + 2], CM.map_planes[CM.numplanes + 3],
				CM.map_planes[CM.numplanes + 4], CM.map_planes[CM.numplanes + 5], CM.map_planes[CM.numplanes + 6], CM.map_planes[CM.numplanes + 7],
				CM.map_planes[CM.numplanes + 8], CM.map_planes[CM.numplanes + 9], CM.map_planes[CM.numplanes + 10], CM.map_planes[CM.numplanes + 11],
