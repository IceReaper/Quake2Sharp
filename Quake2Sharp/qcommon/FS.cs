 ...
 */
	public sealed class FS
	{
		...
		public static void CreatePath(string path)
		{
			...
			if (index > 0)
			{
				if (!Directory.CreateDirectory(path.Substring(0, index)).Exists)
				{
		...
		public static int FileLength(string filename)
		{
			...
			for (var i = 0; i < FS.fs_links.Count; i++)
			{
				...
				if (filename.Substring(0, link.fromlength) == link.from.Substring(0, link.fromlength))
				{
		...
		public static BinaryReader FOpenFile(string filename)
		{
			...
			for (var i = 0; i < FS.fs_links.Count; i++)
			{
				...
				if (filename.Substring(0, link.fromlength) == link.from.Substring(0, link.fromlength))
				{
					...
					{
						//Com.DPrintf ("link file: " + netpath +'\n');
						return new BinaryReader(File.OpenRead(netpath));
					}
			...
			for (search = FS.fs_searchpaths; search != null; search = search.next)
			{
				...
				if (search.pack != null)
				{
					...
					if (entry != null)
					{
						...
						return new BinaryReader(new MemoryStream(file));
					}
				}
				else
				{
				...
				for (var i = 0; i < FS.fs_links.Count; i++)
				{
					...
					if (filename.Substring(0, link.fromlength) == link.from.Substring(0, link.fromlength))
					{
						...
						if (File.Exists(netpath))
						{
							...
							input.Read(tmp);
							buffer = new BinaryReader(new MemoryStream(tmp));
							input.Close();
				...
				for (search = FS.fs_searchpaths; search != null; search = search.next)
				{
					...
					if (search.pack != null)
					{
						...
						if (entry != null)
						{
							...
							return new BinaryReader(new MemoryStream(tmp));
						}
					}
					else
					{
		...
		private static pack_t LoadPackFile(string packfile)
		{
			...
			try
			{
				...
				newfiles = new Dictionary<string, packfile_t>();
