...
namespace Quake2Sharp.client
{
	...
	public sealed class Cl
	{
		...
		{
			new[] {"timescale", "1"}, new[] {"timedemo", "0"}, new[] {"r_drawworld", "1"}, new[] {"cl_testlights", "0"}, new[] {"r_fullbright", "0"},
			new[] {"r_drawflat", "0"}, new[] {"paused", "0"}, new[] {"fixedtime", "0"}, new[] {"sw_draworder", "0"}, new[] {"gl_lightmap", "0"},
			new[] {"gl_saturatelighting", "0"}, new string[] {null, null}
		};

		private static readonly CheatvarT[] Cheatvars;

		static Cl()
		{
			Cl.Cheatvars = new CheatvarT[Cl.Cheatvarsinfo.Length];

			for (var n = 0; n < Cl.Cheatvarsinfo.Length; n++)
				Cl.Cheatvars[n] = new CheatvarT {Name = Cl.Cheatvarsinfo[n][0], Value = Cl.Cheatvarsinfo[n][1]};
		}
		...
		private static readonly Action RecordF = () =>
		{
			try
			{
				...
				FS.CreatePath(name);
				Globals.cls.demofile = new BinaryWriter(File.OpenWrite(name));
		...
		private static readonly Action DisconnectF = () =>
		{
