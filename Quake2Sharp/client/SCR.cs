 ...
 */
	public sealed class SCR
	{
		...
		{
			Cvar.SetValue("viewsize", SCR.scr_viewsize.value + 10);
		}
		...
		private static void Sky_f()
		{
			...
			if (Cmd.Argc() > 2)
				rotate = float.Parse(Cmd.Argv(2), CultureInfo.InvariantCulture);
			else
				rotate = 0;

			if (Cmd.Argc() == 6)
			{
				axis[0] = float.Parse(Cmd.Argv(3), CultureInfo.InvariantCulture);
				axis[1] = float.Parse(Cmd.Argv(4), CultureInfo.InvariantCulture);
				axis[2] = float.Parse(Cmd.Argv(5), CultureInfo.InvariantCulture);
			}
			else
			{
				axis[0] = 0;
			...
			Globals.re.SetSky(Cmd.Argv(1), rotate, axis);
		}
		...
		public static void Init()
		{
			...
			SCR.scr_conspeed = Cvar.Get("scr_conspeed", "3", 0);
			SCR.scr_showturtle = Cvar.Get("scr_showturtle", "0", 0);
			SCR.scr_showpause = Cvar.Get("scr_showpause", "1", 0);
			SCR.scr_centertime = Cvar.Get("scr_centertime", "2.5", 0);
			SCR.scr_printspeed = Cvar.Get("scr_printspeed", "8", 0);
			SCR.scr_netgraph = Cvar.Get("netgraph", "1", 0);
			SCR.scr_timegraph = Cvar.Get("timegraph", "1", 0);
			SCR.scr_debuggraph = Cvar.Get("debuggraph", "1", 0);
			SCR.scr_graphheight = Cvar.Get("graphheight", "32", 0);
			SCR.scr_graphscale = Cvar.Get("graphscale", "1", 0);
			SCR.scr_graphshift = Cvar.Get("graphshift", "0", 0);
			SCR.scr_drawall = Cvar.Get("scr_drawall", "1", 0);
			SCR.fps = Cvar.Get("fps", "0", 0);
		...
		public static void BeginLoadingPlaque()
		{
			...
			SCR.UpdateScreen();
			Globals.cls.disable_screen = Timer.Milliseconds();
			...
			while (parser.hasNext())
			{
				...
				if (parser.tokenEquals("pic"))
				{
					...
					if (value >= Defines.MAX_IMAGES)
						Com.Error(Defines.ERR_DROP, "Pic >= MAX_IMAGES");
				...
				if (parser.tokenEquals("client"))
				{
					...
					if (value >= Defines.MAX_CLIENTS || value < 0)
						Com.Error(Defines.ERR_DROP, "client >= MAX_CLIENTS");
				...
				if (parser.tokenEquals("ctf"))
				{
					...
					var block = Com.sprintf("%3d %3d %-12.12s", new Vargs(3).add(score).add(ping).add(ci.name));
		...
		private static void UpdateScreen2()
		{
			...
			if (Globals.cls.disable_screen != 0)
			{
				if (Timer.Milliseconds() - Globals.cls.disable_screen > 120000)
				{
					Globals.cls.disable_screen = 0;
		...
		private static readonly Action updateScreenCallback = () =>
		{
			SCR.UpdateScreen2();
		};
     ...
     */
		private static byte[] ReadNextFrame()
		{
			...
			if (size > SCR.compressed.Length || size < 1)
				Com.Error(Defines.ERR_DROP, "Bad compressed frame size:" + size);
     ...
     */
		public static void PlayCinematic(string arg)
		{
			...
			var name = "video/" + arg;
			Globals.cl.cinematic_file = FS.LoadMappedFile(name);
			...
			Globals.cl.cinematictime = Timer.Milliseconds();
		}
	}
}
