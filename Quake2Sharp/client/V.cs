	...
	public sealed class V
	{
		...
		public static void AddLightStyle(int style, float r, float g, float b)
		{
			...
			if (style < 0 || style > Defines.MAX_LIGHTSTYLES)
				Com.Error(Defines.ERR_DROP, "Bad light style " + style);
		...
		public static void RenderView(float stereo_separation)
		{
			...
			if (V.cl_stats.value != 0.0f)
				Com.Printf("ent:%i  lt:%i  part:%i\n", new Vargs(3).add(V.r_numentities).add(V.r_numdlights).add(V.r_numparticles));
		...
		private static readonly Action Viewpos_f = () =>
		{
			Com.Printf(
				"(%i %i %i) : %i\n",
				new Vargs(4).add((int) Globals.cl.refdef.vieworg[0])
					.add((int) Globals.cl.refdef.vieworg[1])
					.add((int) Globals.cl.refdef.vieworg[2])
					.add((int) Globals.cl.refdef.viewangles[Defines.YAW])
			);
		};
