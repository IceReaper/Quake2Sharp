	...
	public abstract class OpenTkDriver : OpenTkGL, GLDriver
	{
		...
		public unsafe int setMode(Size dim, int mode, bool fullscreen)
		{
			...
			if (this.oldDisplayMode == null)
			{
				...
				this.oldDisplayMode = new DisplayMode(
					videoMode.Width,
					videoMode.Height,
					videoMode.RefreshRate,
					videoMode.RedBits + videoMode.GreenBits + videoMode.BlueBits
			...
			if (fullscreen)
			{
				...
				this.window = new GameWindow(
					...
					{
						Profile = ContextProfile.Compatability, Size = new Vector2i(displayMode.Width, displayMode.Height), IsFullscreen = true
					}
				);
			...
			else
			{
				this.window = new GameWindow(
					GameWindowSettings.Default,
					new NativeWindowSettings {Profile = ContextProfile.Compatability, Size = new Vector2i(windowSize.Width, windowSize.Height)}
				);
