	...
	public class SV_CCMDS
	{
		...
		*/
		public static void SV_WriteLevelFile()
		{
			...
			try
			{
				f = new BinaryWriter(File.OpenWrite(name));
		...
		*/
		public static void SV_WriteServerFile(bool autosave)
		{
			...
			try
			{
				...
				{
					var c = DateTime.Now;
					comment = Com.sprintf("%2i:%2i %2i/%2i  ", new Vargs().add(c.Hour).add(c.Minute).add(c.Month).add(c.Day));
		...
		*/
		public static void SV_ServerRecord_f()
		{
			...
			try
			{
				SV_INIT.svs.demofile = new BinaryWriter(File.OpenWrite(name));
			}
			catch (Exception)
			{
