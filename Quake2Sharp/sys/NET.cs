...
namespace Quake2Sharp.sys
{
	...
	{
		private readonly static int MAX_LOOPBACK = 4;
		...
		private static UdpSocket Socket(string ip, int port)
		{
			...
			try
			{
				if (string.IsNullOrEmpty(ip) || ip.Equals("localhost"))
					newsocket = port == Defines.PORT_ANY ? new UdpSocket(new IPEndPoint(IPAddress.Any, 0)) : new UdpSocket(new IPEndPoint(IPAddress.Any, port));
				else
					newsocket = new UdpSocket(new IPEndPoint(IPAddress.Parse(ip), port));
			}
			catch (Exception e)
			{
				Com.Println("Error: " + e);
