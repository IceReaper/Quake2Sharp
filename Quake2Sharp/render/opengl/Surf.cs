 ...
 */
	public partial class Basic
	{
		...
		private static readonly int BLOCK_WIDTH = 128;
		private static readonly int BLOCK_HEIGHT = 128;

		private static readonly int MAX_LIGHTMAPS = 128;

		private int c_visible_lightmaps;
		private int c_visible_textures;

		private static readonly int GL_LIGHTMAP_FORMAT = QGLConst.GL_RGBA;

		private class gllightmapstate_t
		{
			public int internal_format;
			public int current_lightmap_texture;

			public readonly msurface_t[] lightmap_surfaces = new msurface_t[Basic.MAX_LIGHTMAPS];
			public readonly int[] allocated = new int[Basic.BLOCK_WIDTH];

			// the lightmap texture data needs to be kept in
			// main memory so texsubimage can update properly
			public readonly int[] lightmap_buffer = new int[Basic.BLOCK_WIDTH * Basic.BLOCK_HEIGHT];

			public gllightmapstate_t()
			{
				for (var i = 0; i < Basic.MAX_LIGHTMAPS; i++)
		...
		*/
		...
		{
			Base.gl.glBegin(QGLConst.GL_POLYGON);

			for (var i = 0; i < p.numverts; i++)
			{
				Base.gl.glTexCoord2f(p.s1(i), p.t1(i));
		...
		*/
		private void DrawGLFlowingPoly(msurface_t fa)
		{
			...
			for (var i = 0; i < p.numverts; i++)
			{
				Base.gl.glTexCoord2f(p.s1(i) + scroll, p.t1(i));
				Base.gl.glVertex3f(p.x(i), p.y(i), p.z(i));
			}
			...
			for (var i = 0; i < Basic.MAX_LIGHTMAPS; i++)
			{
				...
				for (surf = this.gl_lms.lightmap_surfaces[i]; surf != null; surf = surf.lightmapchain)
				{
					...
					for (; p != null; p = p.chain)
					{
						...
						{
							Base.gl.glBegin(QGLConst.GL_LINE_STRIP);
							Base.gl.glVertex3f(p.x(0), p.y(0), p.z(0));
							Base.gl.glVertex3f(p.x(j - 1), p.y(j - 1), p.z(j - 1));
							Base.gl.glVertex3f(p.x(j), p.y(j), p.z(j));
							Base.gl.glVertex3f(p.x(0), p.y(0), p.z(0));
							Base.gl.glEnd();
						}
					}
		...
		private void DrawGLPolyChain(glpoly_t p, float soffset, float toffset)
		{
			if (soffset == 0 && toffset == 0)
			{
				for (; p != null; p = p.chain)
				{
					...
					{
						Base.gl.glTexCoord2f(p.s2(j), p.t2(j));
						Base.gl.glVertex3f(p.x(j), p.y(j), p.z(j));
					}
			...
			else
			{
				for (; p != null; p = p.chain)
				{
					...
					for (var j = 0; j < p.numverts; j++)
					{
						Base.gl.glTexCoord2f(p.s2(j) - soffset, p.t2(j) - toffset);
						Base.gl.glVertex3f(p.x(j), p.y(j), p.z(j));
					}
		...
		private void R_BlendLightmaps()
		{
			...
			*/
			if (this.gl_lightmap.value == 0)
			{
				...
				{
					Base.gl.glBlendFunc(QGLConst.GL_ONE, QGLConst.GL_ONE);
				}
				else
				{
					...
					if (format != '0')
					{
						...
						{
							case 'I':
								Base.gl.glBlendFunc(QGLConst.GL_ZERO, QGLConst.GL_SRC_COLOR);
			...
			*/
			for (i = 1; i < Basic.MAX_LIGHTMAPS; i++)
			{
			...
			if (this.gl_dynamic.value != 0)
			{
				...
				for (surf = this.gl_lms.lightmap_surfaces[0]; surf != null; surf = surf.lightmapchain)
				{
					...
					if (this.LM_AllocBlock(smax, tmax, lightPos))
					{
						...
						var position = surf.dlight_t * Basic.BLOCK_WIDTH + surf.dlight_s;

						this.R_BuildLightMap(surf, @base, position, Basic.BLOCK_WIDTH);
					}
					else
					{
						msurface_t drawsurf;
		...
		*/
		private void R_RenderBrushPoly(msurface_t fa)
		{
			...
			if ((fa.flags & Defines.SURF_DRAWTURB) != 0)
			{
				...
				Base.gl.glColor4f(this.gl_state.inverse_intensity, this.gl_state.inverse_intensity, this.gl_state.inverse_intensity, 1.0F);
		...
		protected void R_DrawAlphaSurfaces()
		{
			...
			for (s = this.r_alpha_surfaces; s != null; s = s.texturechain)
			{
				...
				if ((s.texinfo.flags & Defines.SURF_TRANS33) != 0)
					Base.gl.glColor4f(intens, intens, intens, 0.33f);
				else if ((s.texinfo.flags & Defines.SURF_TRANS66) != 0)
					Base.gl.glColor4f(intens, intens, intens, 0.66f);
				else
					Base.gl.glColor4f(intens, intens, intens, 1);
		...
		private void GL_RenderLightmappedPoly(msurface_t surf)
		{
			...
			if (is_dynamic)
			{
				...
				if (((surf.styles[map] & 0xFF) >= 32 || surf.styles[map] == 0) && (surf.dlightframe != this.r_framecount))
				{
					...
					this.GL_MBind(QGLConst.GL_TEXTURE1, this.gl_state.lightmap_textures + surf.lightmaptexturenum);
				...
				else
				{
					...
					this.GL_MBind(QGLConst.GL_TEXTURE1, this.gl_state.lightmap_textures + 0);
				...
				this.GL_MBind(QGLConst.GL_TEXTURE0, image.texnum);
				this.GL_MBind(QGLConst.GL_TEXTURE1, this.gl_state.lightmap_textures + lmtex);
				...
				if ((surf.texinfo.flags & Defines.SURF_FLOWING) != 0)
				{
					...
					for (p = surf.polys; p != null; p = p.chain)
					{
						...
						for (var i = 0; i < nv; i++)
						{
							Base.gl.glMultiTexCoord2f(QGLConst.GL_TEXTURE0, p.s1(i) + scroll, p.t1(i));
							Base.gl.glMultiTexCoord2f(QGLConst.GL_TEXTURE1, p.s2(i), p.t2(i));
				...
				else
				{
					for (p = surf.polys; p != null; p = p.chain)
					{
						...
						for (var i = 0; i < nv; i++)
						{
							Base.gl.glMultiTexCoord2f(QGLConst.GL_TEXTURE0, p.s1(i), p.t1(i));
							Base.gl.glMultiTexCoord2f(QGLConst.GL_TEXTURE1, p.s2(i), p.t2(i));
		...
		*/
		...
		private void LM_UploadBlock(bool dynamic)
		{
			...
			else
			{
				...
				if (++this.gl_lms.current_lightmap_texture == Basic.MAX_LIGHTMAPS)
		...
		private bool LM_AllocBlock(int w, int h, pos_t pos)
		{
			...
			best = Basic.BLOCK_HEIGHT;

			for (i = 0; i < Basic.BLOCK_WIDTH - w; i++)
			{
				best2 = 0;
		...
		*/
		protected void GL_BuildPolygonFromSurface(msurface_t fa)
		{
			...
			for (var i = 0; i < lnumverts; i++)
			{
				...
				s += fa.light_s * 16;
				s += 8;
				s /= Basic.BLOCK_WIDTH * 16; //fa.texinfo.texture.width;

				t = Math3D.DotProduct(vec, fa.texinfo.vecs[1]) + fa.texinfo.vecs[1][3];
				t -= fa.texturemins[1];
				t += fa.light_t * 16;
				t += 8;
				t /= Basic.BLOCK_HEIGHT * 16; //fa.texinfo.texture.height;
		...
		*/
		protected void GL_CreateSurfaceLightmap(msurface_t surf)
		{
			...
			var basep = (surf.light_t * Basic.BLOCK_WIDTH + surf.light_s);
			@base = this.gl_lms.lightmap_buffer;
			var position = basep;
		...
		*/
		protected void GL_BeginBuildingLightmaps(model_t m)
		{
			...
			*/
			...
			{
				this.gl_lms.internal_format = QGLConst.GL_INTENSITY8;
			}
			else if (format == 'L')
			{
				this.gl_lms.internal_format = QGLConst.GL_LUMINANCE8;
			}
			else
			{
