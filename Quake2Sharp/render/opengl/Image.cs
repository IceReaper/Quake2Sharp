 ...
 */
	public partial class Basic
	{
		...
		protected int gl_filter_min = QGLConst.GL_LINEAR_MIPMAP_NEAREST;
		protected int gl_filter_max = QGLConst.GL_LINEAR;
		...
		protected void ImageConstructor()
		{
			...
			for (var i = 0; i < this.scrap_allocated.Length; i++)
				this.scrap_allocated[i] = new int[Basic.IMG_BLOCK_WIDTH];

			for (var i = 0; i < this.scrap_texels.Length; i++)
				this.scrap_texels[i] = new byte[Basic.IMG_BLOCK_WIDTH * Basic.IMG_BLOCK_HEIGHT];
		}

		protected void GL_SetTexturePalette(int[] palette)
		{
			if (!(palette != null && palette.Length == 256))
				throw new Exception("int palette[256] bug");
			...
			if (this.qglColorTableEXT && this.gl_ext_palettedtexture.value != 0.0f)
			{
				...
				Base.gl.glColorTable(QGLConst.GL_SHARED_TEXTURE_PALETTE_EXT, QGLConst.GL_RGB, 256, QGLConst.GL_RGB, QGLConst.GL_UNSIGNED_BYTE, temptable);
			}
		}
		...
		protected void GL_EnableMultitexture(bool enable)
		{
			...
			if (enable)
			{
				this.GL_SelectTexture(QGLConst.GL_TEXTURE1);
			...
			this.GL_SelectTexture(QGLConst.GL_TEXTURE0);
			this.GL_TexEnv(QGLConst.GL_REPLACE);
		}
		...
		protected void GL_SelectTexture(int texture /* GLenum */)
		{
			...
			if (texture == QGLConst.GL_TEXTURE0)
			{
				tmu = 0;
			}
			else
			...
			if (this.qglSelectTextureSGIS)
			{
				...
				Base.gl.glActiveTextureARB(texture);
			}
			else if (this.qglActiveTextureARB)
			{
				Base.gl.glActiveTextureARB(texture);
				Base.gl.glClientActiveTextureARB(texture);
			}
		}
		...
		)
		{
			...
			{
				Base.gl.glTexEnvi(QGLConst.GL_TEXTURE_ENV, QGLConst.GL_TEXTURE_ENV_MODE, mode);
		...
		protected void GL_Bind(int texnum)
		{
			...
			Base.gl.glBindTexture(QGLConst.GL_TEXTURE_2D, texnum);
		}
		...
		protected void GL_MBind(int target /* GLenum */, int texnum)
		{
			...
			if (target == QGLConst.GL_TEXTURE0)
			{
		...
		{
			new("GL_NEAREST", QGLConst.GL_NEAREST, QGLConst.GL_NEAREST), new("GL_LINEAR", QGLConst.GL_LINEAR, QGLConst.GL_LINEAR),
			new("GL_NEAREST_MIPMAP_NEAREST", QGLConst.GL_NEAREST_MIPMAP_NEAREST, QGLConst.GL_NEAREST),
			new("GL_LINEAR_MIPMAP_NEAREST", QGLConst.GL_LINEAR_MIPMAP_NEAREST, QGLConst.GL_LINEAR),
			new("GL_NEAREST_MIPMAP_LINEAR", QGLConst.GL_NEAREST_MIPMAP_LINEAR, QGLConst.GL_NEAREST),
			new("GL_LINEAR_MIPMAP_LINEAR", QGLConst.GL_LINEAR_MIPMAP_LINEAR, QGLConst.GL_LINEAR)
		};

		private static readonly int NUM_GL_MODES = Basic.modes.Length;

		// gltmode_t
		private class gltmode_t
		{
			public readonly string name;
		...
		{
			new("default", 4), new("GL_RGBA", QGLConst.GL_RGBA), new("GL_RGBA8", QGLConst.GL_RGBA8), new("GL_RGB5_A1", QGLConst.GL_RGB5_A1),
			new("GL_RGBA4", QGLConst.GL_RGBA4), new("GL_RGBA2", QGLConst.GL_RGBA2),
		};

		private static readonly int NUM_GL_ALPHA_MODES = Basic.gl_alpha_modes.Length;

		private static readonly gltmode_t[] gl_solid_modes =
		{
			new("default", 3), new("GL_RGB", QGLConst.GL_RGB), new("GL_RGB8", QGLConst.GL_RGB8), new("GL_RGB5", QGLConst.GL_RGB5),
			new("GL_RGB4", QGLConst.GL_RGB4), new("GL_R3_G3_B2", QGLConst.GL_R3_G3_B2),
		};

		private static readonly int NUM_GL_SOLID_MODES = Basic.gl_solid_modes.Length;
		...
		*/
		protected void GL_TextureMode(string @string)
		{
			...
			for (i = 0; i < Basic.NUM_GL_MODES; i++)
			{
				if (Basic.modes[i].name.Equals(@string, StringComparison.OrdinalIgnoreCase))
					break;
			}

			if (i == Basic.NUM_GL_MODES)
			{
			...
			this.gl_filter_min = Basic.modes[i].minimize;
			this.gl_filter_max = Basic.modes[i].maximize;
			...
			for (i = 0; i < this.numgltextures; i++)
			{
				...
				if (glt.type != Base.it_pic && glt.type != Base.it_sky)
				{
					...
					Base.gl.glTexParameteri(QGLConst.GL_TEXTURE_2D, QGLConst.GL_TEXTURE_MIN_FILTER, this.gl_filter_min);
					Base.gl.glTexParameteri(QGLConst.GL_TEXTURE_2D, QGLConst.GL_TEXTURE_MAG_FILTER, this.gl_filter_max);
				}
			}
		}
		...
		*/
		protected void GL_TextureAlphaMode(string @string)
		{
			...
			for (i = 0; i < Basic.NUM_GL_ALPHA_MODES; i++)
			{
				if (Basic.gl_alpha_modes[i].name.Equals(@string, StringComparison.OrdinalIgnoreCase))
					break;
			}

			if (i == Basic.NUM_GL_ALPHA_MODES)
			{
			...
			this.gl_tex_alpha_format = Basic.gl_alpha_modes[i].mode;
		}
		...
		*/
		protected void GL_TextureSolidMode(string @string)
		{
			...
			for (i = 0; i < Basic.NUM_GL_SOLID_MODES; i++)
			{
				if (Basic.gl_solid_modes[i].name.Equals(@string, StringComparison.OrdinalIgnoreCase))
					break;
			}

			if (i == Basic.NUM_GL_SOLID_MODES)
			{
			...
			this.gl_tex_solid_format = Basic.gl_solid_modes[i].mode;
		}
		...
		*/
		...
		private readonly int[][] scrap_allocated = new int[Basic.MAX_SCRAPS][];
		private readonly byte[][] scrap_texels = new byte[Basic.MAX_SCRAPS][];
		...
		private int Scrap_AllocBlock(int w, int h, pos_t pos)
		{
			...
			for (texnum = 0; texnum < Basic.MAX_SCRAPS; texnum++)
			{
				best = Basic.IMG_BLOCK_HEIGHT;

				for (i = 0; i < Basic.IMG_BLOCK_WIDTH - w; i++)
				{
					best2 = 0;
				...
				if (best + h > Basic.IMG_BLOCK_HEIGHT)
					continue;
		...
		protected void Scrap_Upload()
		{
			...
			this.GL_Upload8(this.scrap_texels[0], Basic.IMG_BLOCK_WIDTH, Basic.IMG_BLOCK_HEIGHT, false, false);
		...
		*/
		private byte[] LoadTGA(string name, ref Size dim)
		{
			...
			buf_p = new BinaryReader(new MemoryStream(targa_header.data, targa_header.id_length, targa_header.data.Length - targa_header.id_length));
		...
		*/
		...
		private static readonly int FLOODFILL_FIFO_MASK = Basic.FLOODFILL_FIFO_SIZE - 1;
		...
		//		else if (pos[off] != 255) fdc = pos[off];
		//	}
		private static readonly floodfill_t[] fifo = new floodfill_t[Basic.FLOODFILL_FIFO_SIZE];

		static Basic()
		{
			for (var j = 0; j < Basic.fifo.Length; j++)
			{
				Basic.fifo[j] = new floodfill_t();
			}
		}
		...
		private void R_FloodFillSkin(byte[] skin, int skinwidth, int skinheight)
		{
			...
			if (filledcolor == -1)
			{
				...
				for (i = 0; i < 256; ++i)
					...
					// TODO check this
					if ((uint)Basic.d_8to24table[i] == 0xFF000000)
					{
						// alpha 1.0
			...
			Basic.fifo[inpt].x = 0;
			Basic.fifo[inpt].y = 0;
			inpt = (inpt + 1) & Basic.FLOODFILL_FIFO_MASK;

			while (outpt != inpt)
			{
				int x = Basic.fifo[outpt].x;
				int y = Basic.fifo[outpt].y;
				var fdc = filledcolor;
				...
				//
				outpt = (outpt + 1) & Basic.FLOODFILL_FIFO_MASK;
				...
				if (x > 0)
				{
					...
					if (skin[pos + off] == (byte) fillcolor)
					{
						...
						Basic.fifo[inpt].x = (short) (x + dx);
						Basic.fifo[inpt].y = (short) (y + dy);
						inpt = (inpt + 1) & Basic.FLOODFILL_FIFO_MASK;
					}
		...
		*/
		...
		private bool GL_Upload32(int[] data, int width, int height, bool mipmap)
		{
			...
			{
				Base.gl.glTexParameterf(QGLConst.GL_TEXTURE_2D, QGLConst.GL_TEXTURE_MIN_FILTER, this.gl_filter_max);
				Base.gl.glTexParameterf(QGLConst.GL_TEXTURE_2D, QGLConst.GL_TEXTURE_MAG_FILTER, this.gl_filter_max);
			}
		...
		*/
		...
		private bool GL_Upload8(byte[] data, int width, int height, bool mipmap, bool is_sky)
		{
			...
			else
			{
				...
				{
					p = data[i] & 0xff;
					this.trans[i] = Basic.d_8to24table[p];
					...
					if (p == 255)
					{
						...
						this.trans[i] = Basic.d_8to24table[p] & 0x00FFFFFF; // only rgb
					}
				}
		...
		protected image_t GL_LoadPic(string name, byte[] pic, int width, int height, int type, int bits)
		{
			...
			if (image.type == Base.it_pic && bits == 8 && image.width < 64 && image.height < 64)
			{
				...
				for (j = 0; j < image.width; j++, k++)
					this.scrap_texels[texnum][(pos.y + i) * Basic.IMG_BLOCK_WIDTH + pos.x + j] = pic[k];

				image.texnum = Base.TEXNUM_SCRAPS + texnum;
				image.scrap = true;
				image.has_alpha = true;
				image.sl = (pos.x + 0.01f) / (float) Basic.IMG_BLOCK_WIDTH;
				image.sh = (pos.x + image.width - 0.01f) / (float) Basic.IMG_BLOCK_WIDTH;
				image.tl = (pos.y + 0.01f) / (float) Basic.IMG_BLOCK_WIDTH;
				image.th = (pos.y + image.height - 0.01f) / (float) Basic.IMG_BLOCK_WIDTH;
			}
			else
			{
		...
		*/
		protected void GL_FreeUnusedImages()
		{
			...
			for (var i = 0; i < this.numgltextures; i++)
			{
				...
				Base.gl.glDeleteTextures(this.texnum);
				image.clear();
			}
		}
		...
		*/
		protected void Draw_GetPalette()
		{
			...
			for (var i = 0; i < 256; i++)
			{
				...
				Basic.d_8to24table[i] = (255 << 24) | (b << 16) | (g << 8) | (r << 0);
			}

			Basic.d_8to24table[255] &= 0x00FFFFFF; // 255 is transparent

			particle_t.setColorPalette(Basic.d_8to24table);
		}
