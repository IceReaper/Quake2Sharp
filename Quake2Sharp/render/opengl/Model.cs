 ...
 */
	public partial class Basic
	{
		...
		private readonly model_t[] mod_known = new model_t[Basic.MAX_MOD_KNOWN];
		private int mod_numknown;

		// the inline * models from the current map are kept seperate
		private readonly model_t[] mod_inline = new model_t[Basic.MAX_MOD_KNOWN];
		...
		*/
		...
		{
			// init mod_known
			for (var i = 0; i < Basic.MAX_MOD_KNOWN; i++)
			{
		...
		*/
		private model_t Mod_ForName(string name, bool crash)
		{
			...
			{
				if (this.mod_numknown == Basic.MAX_MOD_KNOWN)
		...
		*/
		private void Mod_LoadVisibility(lump_t l)
		{
			...
			this.loadmodel.vis = new qfiles.dvis_t(bb);
		...
		*/
		...
		{
			qfiles.dsprite_t sprout = new(buffer);
