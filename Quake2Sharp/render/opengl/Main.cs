 ...
 * @author cwei
 */
	public partial class Basic : RenderAPI
	{
		...
		*/
		protected bool R_CullBox(float[] mins, float[] maxs)
		{
			if (!(mins.Length == 3 && maxs.Length == 3))
				throw new Exception("vec3_t bug");
		...
		{
			Base.gl.glTranslatef(e.origin[0], e.origin[1], e.origin[2]);

			Base.gl.glRotatef(e.angles[1], 0, 0, 1);
			Base.gl.glRotatef(-e.angles[0], 0, 1, 0);
			Base.gl.glRotatef(-e.angles[2], 1, 0, 0);
		}
		...
		*/
		private void R_DrawSpriteModel(entity_t e)
		{
			...
			psprite = (qfiles.dsprite_t) this.currentmodel.extradata;
			...
			Base.gl.glColor4f(1, 1, 1, alpha);

			this.GL_Bind(this.currentmodel.skins[e.frame].texnum);

			this.GL_TexEnv(QGLConst.GL_MODULATE);
		...
		*/
		private void R_DrawNullModel()
		{
			...
			Base.gl.glPushMatrix();
			this.R_RotateForEntity(this.currententity);

			Base.gl.glDisable(QGLConst.GL_TEXTURE_2D);
			Base.gl.glColor3f(shadelight[0], shadelight[1], shadelight[2]);

			//		// this replaces the TRIANGLE_FAN
			//		glut.glutWireCube(gl, 20);

			Base.gl.glBegin(QGLConst.GL_TRIANGLE_FAN);
			Base.gl.glVertex3f(0, 0, -16);
			int i;

			for (i = 0; i <= 4; i++)
			{
				Base.gl.glVertex3f((float) (16.0f * Math.Cos(i * Math.PI / 2)), (float) (16.0f * Math.Sin(i * Math.PI / 2)), 0.0f);
			}
		...
		*/
		private void R_DrawEntitiesOnList()
		{
			...
			Base.gl.glDepthMask(false); // no z writes
			...
			Base.gl.glDepthMask(true); // back to writing
		}
		...
		*/
		private void GL_DrawParticles(int num_particles)
		{
			...
			Base.gl.glDepthMask(false); // no z buffering
			Base.gl.glEnable(QGLConst.GL_BLEND);
			this.GL_TexEnv(QGLConst.GL_MODULATE);

			Base.gl.glBegin(QGLConst.GL_TRIANGLES);
			...
			for (int j = 0, i = 0; i < num_particles; i++)
			{
				...
				Base.gl.glColor4ub((byte) ((color >> 0) & 0xFF), (byte) ((color >> 8) & 0xFF), (byte) ((color >> 16) & 0xFF), (byte) ((color >> 24) & 0xFF));

				// first vertex
				Base.gl.glTexCoord2f(0.0625f, 0.0625f);
				Base.gl.glVertex3f(origin_x, origin_y, origin_z);

				// second vertex
				Base.gl.glTexCoord2f(1.0625f, 0.0625f);
				Base.gl.glVertex3f(origin_x + up[0] * scale, origin_y + up[1] * scale, origin_z + up[2] * scale);

				// third vertex
				Base.gl.glTexCoord2f(0.0625f, 1.0625f);
				Base.gl.glVertex3f(origin_x + right[0] * scale, origin_y + right[1] * scale, origin_z + right[2] * scale);
			}
			...
			Base.gl.glDisable(QGLConst.GL_BLEND);
			Base.gl.glColor4f(1, 1, 1, 1);
			Base.gl.glDepthMask(true); // back to normal Z buffering
			this.GL_TexEnv(QGLConst.GL_REPLACE);
		}
		...
		*/
		private void R_DrawParticles()
		{
			...
			{
				Base.gl.glEnableClientState(QGLConst.GL_VERTEX_ARRAY);
				Base.gl.glVertexPointer(3, 0, particle_t.vertexArray);
				Base.gl.glEnableClientState(QGLConst.GL_COLOR_ARRAY);
				Base.gl.glColorPointer(4, true, 0, particle_t.colorArray);

				Base.gl.glDepthMask(false);
				Base.gl.glEnable(QGLConst.GL_BLEND);
				Base.gl.glDisable(QGLConst.GL_TEXTURE_2D);
				Base.gl.glPointSize(this.gl_particle_size.value);

				Base.gl.glDrawArrays(QGLConst.GL_POINTS, 0, this.r_newrefdef.num_particles);

				Base.gl.glDisableClientState(QGLConst.GL_COLOR_ARRAY);
				Base.gl.glDisableClientState(QGLConst.GL_VERTEX_ARRAY);

				Base.gl.glDisable(QGLConst.GL_BLEND);
				Base.gl.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
				Base.gl.glDepthMask(true);
				Base.gl.glEnable(QGLConst.GL_TEXTURE_2D);
			}
			else
			{
		...
		*/
		private void R_PolyBlend()
		{
			...
			Base.gl.glLoadIdentity();

			// FIXME: get rid of these
			Base.gl.glRotatef(-90, 1, 0, 0); // put Z going up
			Base.gl.glRotatef(90, 0, 0, 1); // put Z going up

			Base.gl.glColor4f(this.v_blend[0], this.v_blend[1], this.v_blend[2], this.v_blend[3]);

			Base.gl.glBegin(QGLConst.GL_QUADS);

			Base.gl.glVertex3f(10, 100, 100);
			Base.gl.glVertex3f(10, -100, 100);
			Base.gl.glVertex3f(10, -100, -100);
			Base.gl.glVertex3f(10, 100, -100);
			Base.gl.glEnd();
		...
		*/
		private void R_SetupFrame()
		{
			...
			if ((this.r_newrefdef.rdflags & Defines.RDF_NOWORLDMODEL) != 0)
			{
				Base.gl.glEnable(QGLConst.GL_SCISSOR_TEST);
				Base.gl.glClearColor(0.3f, 0.3f, 0.3f, 1.0f);
				...
				Base.gl.glClear(QGLConst.GL_COLOR_BUFFER_BIT | QGLConst.GL_DEPTH_BUFFER_BIT);
				Base.gl.glClearColor(1.0f, 0.0f, 0.5f, 0.5f);
				Base.gl.glDisable(QGLConst.GL_SCISSOR_TEST);
			}
		}
		...
		protected void MYgluPerspective(double fovy, double aspect, double zNear, double zFar)
		{
			...
			Base.gl.glFrustum(xmin, xmax, ymin, ymax, zNear, zFar);
		}
		...
		*/
		private void R_SetupGL()
		{
			...
			y2 = (int) Math.Ceiling(
				Base.vid.getHeight() - ((float) this.r_newrefdef.y + this.r_newrefdef.height) * Base.vid.getHeight() / Base.vid.getHeight()
			);

			w = x2 - x;
			h = y - y2;

			Base.gl.glViewport(x, y2, w, h);

			//
			// set up projection matrix
			//
			screenaspect = (float) this.r_newrefdef.width / this.r_newrefdef.height;
			Base.gl.glMatrixMode(QGLConst.GL_PROJECTION);
			Base.gl.glLoadIdentity();
			this.MYgluPerspective(this.r_newrefdef.fov_y, screenaspect, 4, 4096);

			Base.gl.glCullFace(QGLConst.GL_FRONT);

			Base.gl.glMatrixMode(QGLConst.GL_MODELVIEW);
			Base.gl.glLoadIdentity();

			Base.gl.glRotatef(-90, 1, 0, 0); // put Z going up
			Base.gl.glRotatef(90, 0, 0, 1); // put Z going up
			Base.gl.glRotatef(-this.r_newrefdef.viewangles[2], 1, 0, 0);
			Base.gl.glRotatef(-this.r_newrefdef.viewangles[0], 0, 1, 0);
			Base.gl.glRotatef(-this.r_newrefdef.viewangles[1], 0, 0, 1);
			...
			if (this.gl_cull.value != 0.0f)
				Base.gl.glEnable(QGLConst.GL_CULL_FACE);
			else
				Base.gl.glDisable(QGLConst.GL_CULL_FACE);

			Base.gl.glDisable(QGLConst.GL_BLEND);
			Base.gl.glDisable(QGLConst.GL_ALPHA_TEST);
			Base.gl.glEnable(QGLConst.GL_DEPTH_TEST);
		}
		...
		*/
		...
		private void R_Clear()
		{
			if (this.gl_ztrick.value != 0.0f)
			{
				...
				{
					Base.gl.glClear(QGLConst.GL_COLOR_BUFFER_BIT);
				}
				...
				if ((this.trickframe & 1) != 0)
				{
					...
					Base.gl.glDepthFunc(QGLConst.GL_LEQUAL);
				}
				else
				{
					this.gldepthmin = 1;
					this.gldepthmax = 0.5f;
					Base.gl.glDepthFunc(QGLConst.GL_GEQUAL);
				}
			}
			else
			{
				if (this.gl_clear.value != 0.0f)
					...
					Base.gl.glClear(QGLConst.GL_DEPTH_BUFFER_BIT);
			...
			Base.gl.glDepthRange(this.gldepthmin, this.gldepthmax);
		}
		...
		*/
		...
		private void R_SetGL2D()
		{
			...
			Base.gl.glViewport(0, 0, Base.vid.getWidth(), Base.vid.getHeight());
			Base.gl.glMatrixMode(QGLConst.GL_PROJECTION);
			Base.gl.glLoadIdentity();
			Base.gl.glOrtho(0, Base.vid.getWidth(), Base.vid.getHeight(), 0, -99999, 99999);
			Base.gl.glMatrixMode(QGLConst.GL_MODELVIEW);
			Base.gl.glLoadIdentity();
			Base.gl.glDisable(QGLConst.GL_DEPTH_TEST);
			Base.gl.glDisable(QGLConst.GL_CULL_FACE);
		...
		*/
		...
		{
			if (!(Basic.SIN.Length == 256))
				throw new Exception("warpsin table bug");

			// fill r_turbsin
			for (var j = 0; j < 256; j++)
			{
				this.r_turbsin[j] = Basic.SIN[j] * 0.5f;
			}
		...
		public override bool R_Init2()
		{
			...
			*/
			this.gl_config.vendor_string = Base.gl.glGetString(QGLConst.GL_VENDOR);
			VID.Printf(Defines.PRINT_ALL, "GL_VENDOR: " + this.gl_config.vendor_string + '\n');
			this.gl_config.renderer_string = Base.gl.glGetString(QGLConst.GL_RENDERER);
			VID.Printf(Defines.PRINT_ALL, "GL_RENDERER: " + this.gl_config.renderer_string + '\n');
			this.gl_config.version_string = Base.gl.glGetString(QGLConst.GL_VERSION);
			VID.Printf(Defines.PRINT_ALL, "GL_VERSION: " + this.gl_config.version_string + '\n');
			this.gl_config.extensions_string = Base.gl.glGetString(QGLConst.GL_EXTENSIONS);
			...
			*/
			...
			var err = Base.gl.glGetError();

			if (err != QGLConst.GL_NO_ERROR)
				VID.Printf(Defines.PRINT_ALL, "glGetError() = 0x%x\n\t%s\n", new Vargs(2).add(err).add("" + Base.gl.glGetString(err)));
			...
			if (this.gl_drawbuffer.modified)
			{
				...
				if (this.gl_state.camera_separation == 0 || !this.gl_state.stereo_enabled)
				{
					if (this.gl_drawbuffer.@string.Equals("GL_FRONT", StringComparison.OrdinalIgnoreCase))
						Base.gl.glDrawBuffer(QGLConst.GL_FRONT);
					else
						Base.gl.glDrawBuffer(QGLConst.GL_BACK);
				}
			}
		...
		*/
		public override void R_SetPalette(byte[] palette)
		{
			...
			else
			{
				for (i = 0; i < 256; i++)
				{
					this.r_rawpalette[i] = Basic.d_8to24table[i] | unchecked((int) 0xff000000);
				}
			}

			this.GL_SetTexturePalette(this.r_rawpalette);

			Base.gl.glClearColor(0, 0, 0, 0);
			Base.gl.glClear(QGLConst.GL_COLOR_BUFFER_BIT);
			Base.gl.glClearColor(1f, 0f, 0.5f, 0.5f);
		}

		private static readonly int NUM_BEAM_SEGS = 6;

		private readonly float[][] start_points = new float[Basic.NUM_BEAM_SEGS][];

		// array of vec3_t
		private readonly float[][] end_points = new float[Basic.NUM_BEAM_SEGS][]; // array of vec3_t
		...
		*/
		private void R_DrawBeam(entity_t e)
		{
			...
			for (i = 0; i < 6; i++)
			{
				Math3D.RotatePointAroundVector(this.start_points[i], normalized_direction, perpvec, (360.0f / Basic.NUM_BEAM_SEGS) * i);
			...
			r = (Basic.d_8to24table[e.skinnum & 0xFF]) & 0xFF;
			g = (Basic.d_8to24table[e.skinnum & 0xFF] >> 8) & 0xFF;
			b = (Basic.d_8to24table[e.skinnum & 0xFF] >> 16) & 0xFF;

			r *= 1 / 255.0f;
			g *= 1 / 255.0f;
			b *= 1 / 255.0f;

			Base.gl.glColor4f(r, g, b, e.alpha);

			Base.gl.glBegin(QGLConst.GL_TRIANGLE_STRIP);

			float[] v;

			for (i = 0; i < Basic.NUM_BEAM_SEGS; i++)
			{
				v = this.start_points[i];
				Base.gl.glVertex3f(v[0], v[1], v[2]);
				v = this.end_points[i];
				Base.gl.glVertex3f(v[0], v[1], v[2]);
				v = this.start_points[(i + 1) % Basic.NUM_BEAM_SEGS];
				Base.gl.glVertex3f(v[0], v[1], v[2]);
				v = this.end_points[(i + 1) % Basic.NUM_BEAM_SEGS];
				Base.gl.glVertex3f(v[0], v[1], v[2]);
			}
