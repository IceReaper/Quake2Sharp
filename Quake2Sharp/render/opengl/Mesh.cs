	...
	public partial class Basic
	{
		...
		private void GL_LerpVerts(int nverts, int[] ov, int[] v, float[][] lerp, float[] move, float[] frontv, float[] backv)
		{
				...
				!= 0)
			{
				...
				)
				{
					...
					lerp[i][2] = move[2]
						+ (((uint) ovv >> 16) & 0xFF) * backv[2]
						+ (((uint) vv >> 16) & 0xFF) * frontv[2]
		...
		private void GL_DrawAliasFrameLerp(qfiles.dmdl_t paliashdr, float backlerp)
		{
			...
			if (this.gl_vertex_arrays.value != 0.0f)
			{
				...
				Base.gl.glVertexPointer(3, 0, this.vertexArrayBuf);
					...
					!= 0)
				{
					...
					Base.gl.glColor4f(this.shadelight[0], this.shadelight[1], this.shadelight[2], alpha);
				}
				else
				{
					Base.gl.glEnableClientState(QGLConst.GL_COLOR_ARRAY);
					Base.gl.glColorPointer(4, 0, this.colorArrayBuf);
				...
				if (this.qglLockArraysEXT)
					Base.gl.glLockArraysEXT(0, paliashdr.num_xyz);
				...
				while (true)
				{
						...
						!= 0)
					{
						do
						{
							 ...
							 */
							Base.gl.glArrayElement(index_xyz);
						}
						while (--count != 0);
					}
					else
				...
				while (true)
				{
					...
					if ((this.currententity.flags & (Defines.RF_SHELL_RED | Defines.RF_SHELL_GREEN | Defines.RF_SHELL_BLUE)) != 0)
					{
						do
						{
							...
							Base.gl.glVertex3f(tmp[0], tmp[1], tmp[2]);
						}
						while (--count != 0);
					}
					else
					{
						do
						{
							...
							Base.gl.glColor4f(l * this.shadelight[0], l * this.shadelight[1], l * this.shadelight[2], alpha);
							tmp = this.s_lerped[index_xyz];
							Base.gl.glVertex3f(tmp[0], tmp[1], tmp[2]);
						}
						while (--count != 0);
		...
		private bool R_CullAliasModel(float[][] bbox, entity_t e)
		{
			...
			paliashdr = (qfiles.dmdl_t) this.currentmodel.extradata;
		...
		protected void R_DrawAliasModel(entity_t e)
		{
			...
			float[][] bbox =
			{
				new float[] {0, 0, 0}, new float[] {0, 0, 0}, new float[] {0, 0, 0}, new float[] {0, 0, 0}, new float[] {0, 0, 0}, new float[] {0, 0, 0},
			...
			this.shadedots = this.r_avertexnormal_dots[((int) (this.currententity.angles[1] * (Basic.SHADEDOT_QUANT / 360.0))) & (Basic.SHADEDOT_QUANT - 1)];
			...
			if ((this.currententity.flags & Defines.RF_DEPTHHACK) != 0)
				...
				Base.gl.glDepthRange(this.gldepthmin, this.gldepthmin + 0.3 * (this.gldepthmax - this.gldepthmin));
			...
			if ((this.currententity.flags & Defines.RF_WEAPONMODEL) != 0 && (this.r_lefthand.value == 1.0f))
			{
				...
				Base.gl.glCullFace(QGLConst.GL_BACK);
			}
			...
			Base.gl.glShadeModel(QGLConst.GL_SMOOTH);
