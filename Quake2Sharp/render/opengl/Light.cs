 ...
 */
	public partial class Basic
	{
		...
		private void R_RenderDlight(dlight_t light)
		{
			...
			Base.gl.glColor3f(light.color[0] * 0.2f, light.color[1] * 0.2f, light.color[2] * 0.2f);

			for (i = 0; i < 3; i++)
				v[i] = light.origin[i] - this.vpn[i] * rad;

			Base.gl.glVertex3f(v[0], v[1], v[2]);
			Base.gl.glColor3f(0, 0, 0);
		...
		protected void R_MarkLights(dlight_t light, int bit, mnode_t node)
		{
			...
			if (dist > light.intensity - Basic.DLIGHT_CUTOFF)
			{
		...
		private void R_AddDynamicLights(msurface_t surf)
		{
			...
			for (lnum = 0; lnum < this.r_newrefdef.num_dlights; lnum++)
			{
				...
				fminlight = Basic.DLIGHT_CUTOFF; // FIXME: make configurable?
