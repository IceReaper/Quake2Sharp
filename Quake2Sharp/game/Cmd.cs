     ...
     */
		public static void TokenizeString(char[] text, bool macroExpand)
		{
			...
			while (true)
			{
				...
				if (Cmd.cmd_argc == 1)
				{
					Cmd.cmd_args = new string(text, ph.index, len - ph.index);
					Cmd.cmd_args.Trim();
				}
			...
			if (GameBase.flood_msgs.value != 0)
			{
				...
				if (cl.flood_when[i] != 0 && GameBase.level.time - cl.flood_when[i] < GameBase.flood_persecond.value)
				{
					...
					SV_GAME.PF_cprintf(
						ent,
						Defines.PRINT_CHAT,
						"Flood protection:  You can't talk for " + (int) GameBase.flood_waitdelay.value + " seconds.\n"
					);
