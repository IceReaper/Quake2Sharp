...
namespace Quake2Sharp.game.monsters
{
	...
	public class M_Tank
	{
		...
		private static readonly EntInteractAdapter tank_sight = new(
			"tank_sight",
		...
		private static readonly EntThinkAdapter tank_footstep = new(
			"tank_footstep",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter tank_thud = new(
			"tank_thud",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter tank_windup = new(
			"tank_windup",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter tank_idle = new(
			"tank_idle",
			(edict_t self) =>
			{
		...
		private static readonly mframe_t[] tank_frames_stand = new mframe_t[]
		{
		...
		private static readonly EntThinkAdapter tank_stand = new(
			"tank_stand",
			(edict_t self) =>
			{
		...
		// walk
		//
		private static readonly EntThinkAdapter tank_run = new(
			"tank_run",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter tank_walk = new(
			"tank_walk",
			(edict_t self) =>
			{
		...
		private static readonly mframe_t[] tank_frames_start_walk = new mframe_t[]
		{
		...
		private static readonly mframe_t[] tank_frames_walk = new mframe_t[]
		{
			new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 5, null), new(GameAI.ai_walk, 3, null), new(GameAI.ai_walk, 2, null),
			new(GameAI.ai_walk, 5, null), new(GameAI.ai_walk, 5, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, M_Tank.tank_footstep),
			new(GameAI.ai_walk, 3, null), new(GameAI.ai_walk, 5, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 5, null),
			new(GameAI.ai_walk, 7, null), new(GameAI.ai_walk, 7, null), new(GameAI.ai_walk, 6, null), new(GameAI.ai_walk, 6, M_Tank.tank_footstep)
		};

		private static readonly mmove_t tank_move_walk = new(M_Tank.FRAME_walk05, M_Tank.FRAME_walk20, M_Tank.tank_frames_walk, null);

		private static readonly mframe_t[] tank_frames_stop_walk = new mframe_t[]
		{
			new(GameAI.ai_walk, 3, null), new(GameAI.ai_walk, 3, null), new(GameAI.ai_walk, 2, null), new(GameAI.ai_walk, 2, null),
		...
		private static readonly mframe_t[] tank_frames_start_run = new mframe_t[]
		{
		...
		private static readonly mframe_t[] tank_frames_run = new mframe_t[]
		{
			new(GameAI.ai_run, 4, null), new(GameAI.ai_run, 5, null), new(GameAI.ai_run, 3, null), new(GameAI.ai_run, 2, null), new(GameAI.ai_run, 5, null),
			new(GameAI.ai_run, 5, null), new(GameAI.ai_run, 4, null), new(GameAI.ai_run, 4, M_Tank.tank_footstep), new(GameAI.ai_run, 3, null),
			new(GameAI.ai_run, 5, null), new(GameAI.ai_run, 4, null), new(GameAI.ai_run, 5, null), new(GameAI.ai_run, 7, null), new(GameAI.ai_run, 7, null),
			new(GameAI.ai_run, 6, null), new(GameAI.ai_run, 6, M_Tank.tank_footstep)
		};

		private static readonly mmove_t tank_move_run = new(M_Tank.FRAME_walk05, M_Tank.FRAME_walk20, M_Tank.tank_frames_run, null);

		private static readonly mframe_t[] tank_frames_stop_run = new mframe_t[]
		{
			new(GameAI.ai_run, 3, null), new(GameAI.ai_run, 3, null), new(GameAI.ai_run, 2, null), new(GameAI.ai_run, 2, null),
		...
		private static readonly mframe_t[] tank_frames_pain1 = new mframe_t[]
		{
		...
		private static readonly mframe_t[] tank_frames_pain2 = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
		...
		private static readonly mframe_t[] tank_frames_pain3 = new mframe_t[]
		{
			new(GameAI.ai_move, -7, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 2, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 3, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 2, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, M_Tank.tank_footstep)
		};
		...
		private static readonly EntPainAdapter tank_pain = new(
			"tank_pain",
		...
		private static readonly EntThinkAdapter TankBlaster = new(
			"TankBlaster",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter TankStrike = new(
			"TankStrike",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter TankRocket = new(
			"TankRocket",
			(edict_t self) =>
			{
				float[] forward = {0, 0, 0}, right = {0, 0, 0};
		...
		private static readonly EntThinkAdapter TankMachineGun = new(
			"TankMachineGun",
			(edict_t self) =>
			{
				float[] dir = {0, 0, 0};
		...
		private static readonly EntThinkAdapter tank_reattack_blaster = new(
		...
		private static readonly mframe_t[] tank_frames_attack_blast = new mframe_t[]
		{
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, -1, null), new(GameAI.ai_charge, -2, null), new(GameAI.ai_charge, -1, null), new(GameAI.ai_charge, -1, null),
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, M_Tank.TankBlaster),

			// 10
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, M_Tank.TankBlaster), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, M_Tank.TankBlaster) // 16
		};

		private static readonly mmove_t tank_move_attack_blast = new(
			M_Tank.FRAME_attak101,
			M_Tank.FRAME_attak116,
			M_Tank.tank_frames_attack_blast,
			M_Tank.tank_reattack_blaster
		);

		private static readonly mframe_t[] tank_frames_reattack_blast = new mframe_t[]
		{
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, M_Tank.TankBlaster), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, M_Tank.TankBlaster) // 16)
		};

		private static readonly mmove_t tank_move_reattack_blast = new(
			M_Tank.FRAME_attak111,
			M_Tank.FRAME_attak116,
			M_Tank.tank_frames_reattack_blast,
			M_Tank.tank_reattack_blaster
		);

		private static readonly mframe_t[] tank_frames_attack_post_blast = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), // 17)
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 2, null), new(GameAI.ai_move, 3, null), new(GameAI.ai_move, 2, null),
			new(GameAI.ai_move, -2, M_Tank.tank_footstep) // 22
		};

		private static readonly mmove_t tank_move_attack_post_blast = new(
			M_Tank.FRAME_attak117,
			M_Tank.FRAME_attak122,
			M_Tank.tank_frames_attack_post_blast,
			M_Tank.tank_run
		);

		private static readonly EntThinkAdapter tank_poststrike = new(
			"tank_poststrike",
			(edict_t self) =>
			{
				self.enemy = null;
		...
		private static readonly EntThinkAdapter tank_doattack_rocket = new(
		...
		private static readonly EntThinkAdapter tank_refire_rocket = new(
			"tank_refire_rocket",
			(edict_t self) =>
			{
		...
		private static readonly mframe_t[] tank_frames_attack_strike = new mframe_t[]
		{
			new(GameAI.ai_move, 3, null), new(GameAI.ai_move, 2, null), new(GameAI.ai_move, 2, null), new(GameAI.ai_move, 1, null),
			new(GameAI.ai_move, 6, null), new(GameAI.ai_move, 7, null), new(GameAI.ai_move, 9, M_Tank.tank_footstep), new(GameAI.ai_move, 2, null),
			new(GameAI.ai_move, 1, null), new(GameAI.ai_move, 2, null), new(GameAI.ai_move, 2, M_Tank.tank_footstep), new(GameAI.ai_move, 2, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, -2, null), new(GameAI.ai_move, -2, null), new(GameAI.ai_move, 0, M_Tank.tank_windup), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, M_Tank.TankStrike), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, -1, null),
			new(GameAI.ai_move, -1, null), new(GameAI.ai_move, -1, null), new(GameAI.ai_move, -1, null), new(GameAI.ai_move, -1, null),
			new(GameAI.ai_move, -3, null), new(GameAI.ai_move, -10, null), new(GameAI.ai_move, -10, null), new(GameAI.ai_move, -2, null),
			new(GameAI.ai_move, -3, null), new(GameAI.ai_move, -2, M_Tank.tank_footstep)
		};

		private static readonly mmove_t tank_move_attack_strike = new(
			M_Tank.FRAME_attak201,
			M_Tank.FRAME_attak238,
			M_Tank.tank_frames_attack_strike,
			M_Tank.tank_poststrike
		);

		private static readonly mframe_t[] tank_frames_attack_pre_rocket = new mframe_t[]
		{
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
			...
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 1, null), new(GameAI.ai_charge, 2, null), new(GameAI.ai_charge, 7, null),
			new(GameAI.ai_charge, 7, null), new(GameAI.ai_charge, 7, M_Tank.tank_footstep), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
		...
		private static readonly mmove_t tank_move_attack_pre_rocket = new(
			M_Tank.FRAME_attak301,
			M_Tank.FRAME_attak321,
			M_Tank.tank_frames_attack_pre_rocket,
			M_Tank.tank_doattack_rocket
		);

		private static readonly mframe_t[] tank_frames_attack_fire_rocket = new mframe_t[]
		{
			new(GameAI.ai_charge, -3, null), // Loop Start 22 )
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, M_Tank.TankRocket), // 24)
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, M_Tank.TankRocket), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, -1, M_Tank.TankRocket) // 30 Loop End
		};

		private static readonly mmove_t tank_move_attack_fire_rocket = new(
			M_Tank.FRAME_attak322,
			M_Tank.FRAME_attak330,
			M_Tank.tank_frames_attack_fire_rocket,
			M_Tank.tank_refire_rocket
		);

		private static readonly mframe_t[] tank_frames_attack_post_rocket = new mframe_t[]
		{
			new(GameAI.ai_charge, 0, null), // 31)
			new(GameAI.ai_charge, -1, null), new(GameAI.ai_charge, -1, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 2, null),
			new(GameAI.ai_charge, 3, null), new(GameAI.ai_charge, 4, null), new(GameAI.ai_charge, 2, null), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null),

			// 40)

			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, -9, null), new(GameAI.ai_charge, -8, null), new(GameAI.ai_charge, -7, null),
			new(GameAI.ai_charge, -1, null), new(GameAI.ai_charge, -1, M_Tank.tank_footstep), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
		...
		private static readonly mmove_t tank_move_attack_post_rocket = new(
			M_Tank.FRAME_attak331,
			M_Tank.FRAME_attak353,
			M_Tank.tank_frames_attack_post_rocket,
			M_Tank.tank_run
		);

		private static readonly mframe_t[] tank_frames_attack_chain = new mframe_t[]
		{
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null), new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun),
			new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun),
			new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun),
			new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun),
			new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun),
			new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun), new(null, 0, M_Tank.TankMachineGun),
			new(null, 0, M_Tank.TankMachineGun), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null),
			new(GameAI.ai_charge, 0, null), new(GameAI.ai_charge, 0, null)
		};

		private static readonly mmove_t tank_move_attack_chain = new(
			M_Tank.FRAME_attak401,
			M_Tank.FRAME_attak429,
			M_Tank.tank_frames_attack_chain,
			M_Tank.tank_run
		);

		private static readonly EntThinkAdapter tank_attack = new(
			"tank_attack",
			(edict_t self) =>
			{
				float[] vec = {0, 0, 0};
		...
		// death
		//
		private static readonly EntThinkAdapter tank_dead = new(
			"tank_dead",
			(edict_t self) =>
			{
		...
		private static readonly mframe_t[] tank_frames_death1 = new mframe_t[]
		{
			new(GameAI.ai_move, -7, null), new(GameAI.ai_move, -2, null), new(GameAI.ai_move, -2, null), new(GameAI.ai_move, 1, null),
			new(GameAI.ai_move, 3, null), new(GameAI.ai_move, 6, null), new(GameAI.ai_move, 1, null), new(GameAI.ai_move, 1, null),
			new(GameAI.ai_move, 2, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, -2, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, -3, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, -4, null), new(GameAI.ai_move, -6, null),
			new(GameAI.ai_move, -4, null), new(GameAI.ai_move, -5, null), new(GameAI.ai_move, -7, null), new(GameAI.ai_move, -15, M_Tank.tank_thud),
		...
		private static readonly EntDieAdapter tank_die = new(
			"tank_die",
		 ...
		 * Trigger_Spawn Sight
		 */
		public static EntThinkAdapter SP_monster_tank = new(
			"SP_monster_tank",
			(edict_t self) =>
			{
