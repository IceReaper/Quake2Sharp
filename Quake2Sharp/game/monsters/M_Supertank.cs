...
namespace Quake2Sharp.game.monsters
{
	...
	public class M_Supertank
	{
		...
		private static readonly EntThinkAdapter TreadSound = new(
			"TreadSound",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter supertank_search = new(
			"supertank_search",
			(edict_t self) =>
			{
				if (Lib.random() < 0.5)
		...
		private static readonly mframe_t[] supertank_frames_stand = new mframe_t[]
		{
		...
		private static readonly mmove_t supertank_move_stand = new(
			M_Supertank.FRAME_stand_1,
			M_Supertank.FRAME_stand_60,
			M_Supertank.supertank_frames_stand,
			null
		);

		private static readonly EntThinkAdapter supertank_stand = new(
			"supertank_stand",
			(edict_t self) =>
			{
		...
		private static readonly mframe_t[] supertank_frames_run = new mframe_t[]
		{
			new(GameAI.ai_run, 12, M_Supertank.TreadSound), new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null),
			new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null),
			new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null),
			new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null),
			new(GameAI.ai_run, 12, null), new(GameAI.ai_run, 12, null)
		};

		private static readonly mmove_t supertank_move_run = new(
			M_Supertank.FRAME_forwrd_1,
			M_Supertank.FRAME_forwrd_18,
			M_Supertank.supertank_frames_run,
			null
		);
		...
		private static readonly mframe_t[] supertank_frames_forward = new mframe_t[]
		{
			new(GameAI.ai_walk, 4, M_Supertank.TreadSound), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null),
			new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null),
			new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null),
			new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null),
			new(GameAI.ai_walk, 4, null), new(GameAI.ai_walk, 4, null)
		};

		private static readonly mmove_t supertank_move_forward = new(
			M_Supertank.FRAME_forwrd_1,
			M_Supertank.FRAME_forwrd_18,
			M_Supertank.supertank_frames_forward,
			null
		);

		private static EntThinkAdapter supertank_forward = new(
			"supertank_forward",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter supertank_walk = new(
			"supertank_walk",
			(edict_t self) =>
			{
				self.monsterinfo.currentmove = M_Supertank.supertank_move_forward;
		...
		private static readonly EntThinkAdapter supertank_run = new(
			"supertank_run",
			(edict_t self) =>
			{
		...
		// death
		//
		private static readonly EntThinkAdapter supertank_dead = new(
			"supertank_dead",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter supertankRocket = new(
			"supertankRocket",
			(edict_t self) =>
			{
		...
		private static readonly EntThinkAdapter supertankMachineGun = new(
			"supertankMachineGun",
			(edict_t self) =>
			{
				float[] dir = {0, 0, 0};
		...
		private static readonly EntThinkAdapter supertank_attack = new(
			"supertank_attack",
			(edict_t self) =>
			{
				float[] vec = {0, 0, 0};
		...
		private static readonly mframe_t[] supertank_frames_turn_right = new mframe_t[]
		{
			new(GameAI.ai_move, 0, M_Supertank.TreadSound), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
		...
		private static mmove_t supertank_move_turn_right = new(
			M_Supertank.FRAME_right_1,
			M_Supertank.FRAME_right_18,
			M_Supertank.supertank_frames_turn_right,
			M_Supertank.supertank_run
		);

		private static readonly mframe_t[] supertank_frames_turn_left = new mframe_t[]
		{
			new(GameAI.ai_move, 0, M_Supertank.TreadSound), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
		...
		private static mmove_t supertank_move_turn_left = new(
			M_Supertank.FRAME_left_1,
			M_Supertank.FRAME_left_18,
			M_Supertank.supertank_frames_turn_left,
			M_Supertank.supertank_run
		);

		private static readonly mframe_t[] supertank_frames_pain3 = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null)
		};

		private static readonly mmove_t supertank_move_pain3 = new(
			M_Supertank.FRAME_pain3_9,
			M_Supertank.FRAME_pain3_12,
			M_Supertank.supertank_frames_pain3,
			M_Supertank.supertank_run
		);

		private static readonly mframe_t[] supertank_frames_pain2 = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null)
		};

		private static readonly mmove_t supertank_move_pain2 = new(
			M_Supertank.FRAME_pain2_5,
			M_Supertank.FRAME_pain2_8,
			M_Supertank.supertank_frames_pain2,
			M_Supertank.supertank_run
		);

		private static readonly mframe_t[] supertank_frames_pain1 = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null)
		};

		private static readonly mmove_t supertank_move_pain1 = new(
			M_Supertank.FRAME_pain1_1,
			M_Supertank.FRAME_pain1_4,
			M_Supertank.supertank_frames_pain1,
			M_Supertank.supertank_run
		);

		private static readonly mframe_t[] supertank_frames_death1 = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, M_Supertank.BossExplode)
		};

		private static readonly mmove_t supertank_move_death = new(
			M_Supertank.FRAME_death_1,
			M_Supertank.FRAME_death_24,
			M_Supertank.supertank_frames_death1,
			M_Supertank.supertank_dead
		);

		private static readonly mframe_t[] supertank_frames_backward = new mframe_t[]
		{
			new(GameAI.ai_walk, 0, M_Supertank.TreadSound), new(GameAI.ai_walk, 0, null), new(GameAI.ai_walk, 0, null), new(GameAI.ai_walk, 0, null),
		...
		private static mmove_t supertank_move_backward = new(
			M_Supertank.FRAME_backwd_1,
			M_Supertank.FRAME_backwd_18,
			M_Supertank.supertank_frames_backward,
			null
		);

		private static readonly mframe_t[] supertank_frames_attack4 = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null)
		};

		private static mmove_t supertank_move_attack4 = new(
			M_Supertank.FRAME_attak4_1,
			M_Supertank.FRAME_attak4_6,
			M_Supertank.supertank_frames_attack4,
			M_Supertank.supertank_run
		);

		private static readonly mframe_t[] supertank_frames_attack3 = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
		...
		private static mmove_t supertank_move_attack3 = new(
			M_Supertank.FRAME_attak3_1,
			M_Supertank.FRAME_attak3_27,
			M_Supertank.supertank_frames_attack3,
			M_Supertank.supertank_run
		);

		private static readonly mframe_t[] supertank_frames_attack2 = new mframe_t[]
		{
			...
			new(GameAI.ai_charge, 0, M_Supertank.supertankRocket), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, M_Supertank.supertankRocket), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, M_Supertank.supertankRocket), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null)
		};

		private static readonly mmove_t supertank_move_attack2 = new(
			M_Supertank.FRAME_attak2_1,
			M_Supertank.FRAME_attak2_27,
			M_Supertank.supertank_frames_attack2,
			M_Supertank.supertank_run
		);

		private static readonly EntThinkAdapter supertank_reattack1 = new(
			"supertank_reattack1",
			(edict_t self) =>
			{
		...
		private static readonly mframe_t[] supertank_frames_attack1 = new mframe_t[]
		{
			new(GameAI.ai_charge, 0, M_Supertank.supertankMachineGun), new(GameAI.ai_charge, 0, M_Supertank.supertankMachineGun),
			new(GameAI.ai_charge, 0, M_Supertank.supertankMachineGun), new(GameAI.ai_charge, 0, M_Supertank.supertankMachineGun),
			new(GameAI.ai_charge, 0, M_Supertank.supertankMachineGun), new(GameAI.ai_charge, 0, M_Supertank.supertankMachineGun),
		};

		private static readonly mmove_t supertank_move_attack1 = new(
			M_Supertank.FRAME_attak1_1,
			M_Supertank.FRAME_attak1_6,
			M_Supertank.supertank_frames_attack1,
			M_Supertank.supertank_reattack1
		);

		private static readonly mframe_t[] supertank_frames_end_attack1 = new mframe_t[]
		{
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
			new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null), new(GameAI.ai_move, 0, null),
		...
		private static readonly mmove_t supertank_move_end_attack1 = new(
			M_Supertank.FRAME_attak1_7,
			M_Supertank.FRAME_attak1_20,
			M_Supertank.supertank_frames_end_attack1,
			M_Supertank.supertank_run
		);

		private static readonly EntPainAdapter supertank_pain = new(
			"supertank_pain",
		...
		private static readonly EntDieAdapter supertank_die = new(
			"supertank_die",
		 ...
		 * Trigger_Spawn Sight
		 */
		public static EntThinkAdapter SP_monster_supertank = new(
		...
		public static EntThinkAdapter BossExplode = new(
			"BossExplode",
			(edict_t self) =>
			{
				float[] org = {0, 0, 0};
