...
public class SV_WORLD
{
	...
	private static readonly int[] leafs = new int[SV_WORLD.MAX_TOTAL_ENT_LEAFS];
	private static readonly int[] clusters = new int[SV_WORLD.MAX_TOTAL_ENT_LEAFS];

	//===========================================================================
	private static readonly edict_t[] touch = new edict_t[Defines.MAX_EDICTS];

	//===========================================================================
	private static readonly edict_t[] touchlist = new edict_t[Defines.MAX_EDICTS];

	public static void initNodes()
	{
		for (var n = 0; n < Defines.AREA_NODES; n++)
			SV_WORLD.sv_areanodes[n] = new();
	}
	...
	public static void SV_ClearWorld()
	{
		 ...
		 * (int n = 0; n < sv_numareanodes; n++) { clientserver.Com_Printf( "|%3i|%2i|%8.2f
			...
			if (area != 0)
			{
				...
				if (ent.areanum != 0 && ent.areanum != area)
				{
					if (ent.areanum2 != 0 && ent.areanum2 != area && SV_INIT.sv.state == Defines.ss_loading)
						clientserver.Com_DPrintf("Object touching 3 areas at " + ent.absmin[0] + " " + ent.absmin[1] + " " + ent.absmin[2] + "\n");
	...
	public static void SV_AreaEdicts_r(areanode_t node)
	{
		...
		for (l = start.next; l != start; l = next)
		{
			...
			{
				clientserver.Com_Printf("SV_AreaEdicts: MAXCOUNT\n");
	...
	public static int SV_HullForEntity(edict_t ent)
	{
		...
		if (ent.solid == Defines.SOLID_BSP)
		{
			...
			if (null == model)
				clientserver.Com_Error(Defines.ERR_FATAL, "MOVETYPE_PUSH with a non bsp model");
