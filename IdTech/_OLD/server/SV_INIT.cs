...
public class SV_INIT
{
	...
	public static int SV_FindIndex(string name, int start, int max, bool create)
	{
		...
		if (i == max)
			clientserver.Com_Error(Defines.ERR_DROP, "*Index: overflow");
	...
	public static void SV_SpawnServer(string server, string spawnpoint, int serverstate, bool attractloop, bool loadgame)
	{
		...
		clientserver.Com_Printf("------- Server Initialization -------\n");

		clientserver.Com_DPrintf("SpawnServer: " + server + "\n");
		...
		cvar.Cvar_FullSet("mapname", SV_INIT.sv.name, Defines.CVAR_SERVERINFO | Defines.CVAR_NOSET);
	}
	...
	public static void SV_InitGame()
	{
		...
		if (cvar.Cvar_VariableValue("coop") != 0 && cvar.Cvar_VariableValue("deathmatch") != 0)
		{
			clientserver.Com_Printf("Deathmatch and Coop both set, disabling Coop\n");
			cvar.Cvar_FullSet("coop", "0", Defines.CVAR_SERVERINFO | Defines.CVAR_LATCH);
		}
		...
		{
			if (0 == cvar.Cvar_VariableValue("coop"))
				cvar.Cvar_FullSet("deathmatch", "1", Defines.CVAR_SERVERINFO | Defines.CVAR_LATCH);
		}
		...
		if (cvar.Cvar_VariableValue("deathmatch") != 0)
		{
			if (SV_MAIN.maxclients.value <= 1)
				cvar.Cvar_FullSet("maxclients", "8", Defines.CVAR_SERVERINFO | Defines.CVAR_LATCH);
			else if (SV_MAIN.maxclients.value > Defines.MAX_CLIENTS)
				cvar.Cvar_FullSet("maxclients", "" + Defines.MAX_CLIENTS, Defines.CVAR_SERVERINFO | Defines.CVAR_LATCH);
		}
		else if (cvar.Cvar_VariableValue("coop") != 0)
		{
			if (SV_MAIN.maxclients.value <= 1 || SV_MAIN.maxclients.value > 4)
				cvar.Cvar_FullSet("maxclients", "4", Defines.CVAR_SERVERINFO | Defines.CVAR_LATCH);
		}
		else // non-deathmatch, non-coop is one player
			cvar.Cvar_FullSet("maxclients", "1", Defines.CVAR_SERVERINFO | Defines.CVAR_LATCH);
	...
	public static void SV_Map(bool attractloop, string levelstring, bool loadgame)
	{
		...
		if (c != -1)
		{
			cvar.Cvar_Set("nextserver", "gamemap \"" + level[(c + 1)..] + "\"");
			level = level[..c];
		}
		else
		...
		if (cvar.Cvar_VariableValue("coop") != 0 && level.Equals("victory.pcx"))
			cvar.Cvar_Set("nextserver", "gamemap \"*" + SV_INIT.firstmap + "\"");
