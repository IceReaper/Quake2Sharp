...
public class SV_ENTS
{
	...
	public static void SV_FatPVS(float[] org)
	{
		...
		if (count < 1)
			clientserver.Com_Error(Defines.ERR_FATAL, "SV_FatPVS: count < 1");
	...
	public static void SV_BuildClientFrame(client_t client)
	{
		...
		for (e = 1; e < GameBase.num_edicts; e++)
		{
			...
			if (ent.s.number != e)
			{
				clientserver.Com_DPrintf("FIXING ENT.S.NUMBER!!!\n");
				ent.s.number = e;
			}
	...
	public static void SV_RecordDemoMessage()
	{
		...
		{
			clientserver.Com_Printf("Error writing demo file:" + e);
		}
	}
}
