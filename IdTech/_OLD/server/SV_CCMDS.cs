...
public class SV_CCMDS
{
	...
	*/
	public static void SV_SetMaster_f()
	{
		...
		{
			clientserver.Com_Printf("Only dedicated servers use masters.\n");

			return;
		}

		// make sure the server is listed public
		cvar.Cvar_Set("public", "1");
		...
		for (i = 1; i < cmdparser.Cmd_Argc(); i++)
		{
			if (slot == Defines.MAX_MASTERS)
				break;

			if (!NET.StringToAdr(cmdparser.Cmd_Argv(i), SV_MAIN.master_adr[i]))
			{
				clientserver.Com_Printf("Bad address: " + cmdparser.Cmd_Argv(i) + "\n");
			...
			clientserver.Com_Printf("Master server at " + NET.AdrToString(SV_MAIN.master_adr[slot]) + "\n");
			clientserver.Com_Printf("Sending a ping.\n");
	...
	*/
	public static bool SV_SetPlayer()
	{
		...
		s = cmdparser.Cmd_Argv(1);

		// numeric values are just slot numbers
		if (s[0] >= '0' && s[0] <= '9')
		{
			idnum = Lib.atoi(cmdparser.Cmd_Argv(1));

			if (idnum < 0 || idnum >= SV_MAIN.maxclients.value)
			{
				clientserver.Com_Printf("Bad client slot: " + idnum + "\n");
			...
			{
				clientserver.Com_Printf("Client " + idnum + " is not active\n");
		...
		clientserver.Com_Printf("Userid " + s + " is not on the server\n");
	...
	*/
	...
	{
		clientserver.Com_DPrintf("SV_WipeSaveGame(" + savename + ")\n");

		var name = filesystem.FS_Gamedir() + "/save/" + savename + "/server.ssv";

		if (File.Exists(name))
			File.Delete(name);

		name = filesystem.FS_Gamedir() + "/save/" + savename + "/game.ssv";
	...
	*/
	...
	{
		clientserver.Com_DPrintf("SV_CopySaveGame(" + src + "," + dst + ")\n");

		SV_CCMDS.SV_WipeSavegame(dst);

		// copy the savegame over
		var name = filesystem.FS_Gamedir() + "/save/" + src + "/server.ssv";
		var name2 = filesystem.FS_Gamedir() + "/save/" + dst + "/server.ssv";
		filesystem.FS_CreatePath(name2);
		SV_CCMDS.CopyFile(name, name2);

		name = filesystem.FS_Gamedir() + "/save/" + src + "/game.ssv";
		name2 = filesystem.FS_Gamedir() + "/save/" + dst + "/game.ssv";
		SV_CCMDS.CopyFile(name, name2);
		...
		while (found != null)
		{
			...
			name2 = filesystem.FS_Gamedir() + "/save/" + dst + "/" + Path.GetFileName(found);
	...
	*/
	public static void SV_WriteLevelFile()
	{
		...
		clientserver.Com_DPrintf("SV_WriteLevelFile()\n");

		name = filesystem.FS_Gamedir() + "/save/current/" + SV_INIT.sv.name + ".sv2";
		...
		{
			clientserver.Com_Printf("Failed to open " + name + "\n");
			Console.WriteLine(e);
		}

		name = filesystem.FS_Gamedir() + "/save/current/" + SV_INIT.sv.name + ".sav";
	...
	*/
	public static void SV_ReadLevelFile()
	{
		...
		clientserver.Com_DPrintf("SV_ReadLevelFile()\n");
	...
	*/
	public static void SV_WriteServerFile(bool autosave)
	{
		...
		clientserver.Com_DPrintf("SV_WriteServerFile(" + (autosave ? "true" : "false") + ")\n");

		filename = filesystem.FS_Gamedir() + "/save/current/server.ssv";
		...
		try
		{
			...
			for (var i = 0; i < cvar.cvar_vars.Count; i++)
			{
				var = cvar.cvar_vars[i];
				...
				{
					clientserver.Com_Printf("Cvar too long: " + var.name + " = " + var.@string + "\n");
		...
		{
			clientserver.Com_Printf("Couldn't write " + filename + "\n");
		}

		// write game state
		filename = filesystem.FS_Gamedir() + "/save/current/game.ssv";
	...
	*/
	public static void SV_ReadServerFile()
	{
		...
		try
		{
			...
			clientserver.Com_DPrintf("SV_ReadServerFile()\n");
			...
			while (true)
			{
				...
				clientserver.Com_DPrintf("Set " + name + " = " + @string + "\n");
				cvar.Cvar_ForceSet(name, @string);
			}
		...
		{
			clientserver.Com_Printf("Couldn't read file " + filename + "\n");
			Console.WriteLine(e);
		}
	}
	...
	*/
	...
	{
		SV_INIT.SV_Map(true, cmdparser.Cmd_Argv(1), false);
	}
	...
	*/
	public static void SV_GameMap_f()
	{
		if (cmdparser.Cmd_Argc() != 2)
		{
			clientserver.Com_Printf("USAGE: gamemap <map>\n");

			return;
		}

		clientserver.Com_DPrintf("SV_GameMap(" + cmdparser.Cmd_Argv(1) + ")\n");

		filesystem.FS_CreatePath(filesystem.FS_Gamedir() + "/save/current/");

		// check for clearing the current savegame
		var map = cmdparser.Cmd_Argv(1);
		...
		SV_INIT.SV_Map(false, cmdparser.Cmd_Argv(1), false);

		// archive server state
		SV_INIT.svs.mapcmd = cmdparser.Cmd_Argv(1);

		// copy off the level to the autosave slot
		if (0 == frame.dedicated.value)
		{
	...
	*/
	public static void SV_Map_f()
	{
		...
		map = cmdparser.Cmd_Argv(1);

		if (map.IndexOf(".") < 0)
		{
			expanded = "maps/" + map + ".bsp";

			if (filesystem.FS_LoadFile(expanded) == null)
			{
				clientserver.Com_Printf("Can't find " + expanded + "\n");
	...
	*/
	public static void SV_Loadgame_f()
	{
		if (cmdparser.Cmd_Argc() != 2)
		{
			clientserver.Com_Printf("USAGE: loadgame <directory>\n");

			return;
		}

		clientserver.Com_Printf("Loading game...\n");

		var dir = cmdparser.Cmd_Argv(1);

		if (dir.IndexOf("..") > -1 || dir.IndexOf("/") > -1 || dir.IndexOf("\\") > -1)
			clientserver.Com_Printf("Bad savedir.\n");

		// make sure the server.ssv file exists
		var name = filesystem.FS_Gamedir() + "/save/" + cmdparser.Cmd_Argv(1) + "/server.ssv";

		if (!File.Exists(name))
		{
			clientserver.Com_Printf("No such savegame: " + name + "\n");

			return;
		}

		SV_CCMDS.SV_CopySaveGame(cmdparser.Cmd_Argv(1), "current");
	...
	*/
	public static void SV_Savegame_f()
	{
		...
		if (SV_INIT.sv.state != Defines.ss_game)
		{
			clientserver.Com_Printf("You must be in a game to save.\n");
		...
		if (cmdparser.Cmd_Argc() != 2)
		{
			clientserver.Com_Printf("USAGE: savegame <directory>\n");
		...
		{
			clientserver.Com_Printf("Can't savegame in a deathmatch\n");

			return;
		}

		if (0 == Lib.strcmp(cmdparser.Cmd_Argv(1), "current"))
		{
			clientserver.Com_Printf("Can't save to 'current'\n");
		...
		dir = cmdparser.Cmd_Argv(1);

		if (dir.IndexOf("..") > -1 || dir.IndexOf("/") > -1 || dir.IndexOf("\\") > -1)
			clientserver.Com_Printf("Bad savedir.\n");

		clientserver.Com_Printf("Saving game...\n");
		...
		{
			clientserver.Com_Printf("IOError in SV_WriteServerFile: " + e);
		}

		// copy it off
		SV_CCMDS.SV_CopySaveGame("current", dir);
		clientserver.Com_Printf("Done.\n");
	}
	...
	*/
	public static void SV_Kick_f()
	{
		...
		{
			clientserver.Com_Printf("No server running.\n");
		...
		if (cmdparser.Cmd_Argc() != 2)
		{
			clientserver.Com_Printf("Usage: kick <userid>\n");
	...
	*/
	public static void SV_Status_f()
	{
		...
		clientserver.Com_Printf("map              : " + SV_INIT.sv.name + "\n");

		clientserver.Com_Printf("num score ping name            lastmsg address               qport \n");
		clientserver.Com_Printf("--- ----- ---- --------------- ------- --------------------- ------\n");
		...
		for (i = 0; i < SV_MAIN.maxclients.value; i++)
		{
			...
			clientserver.Com_Printf($"{i,3} ");
			clientserver.Com_Printf($"{cl.edict.client.ps.stats[Defines.STAT_FRAGS],5} ");

			if (cl.state == Defines.cs_connected)
				clientserver.Com_Printf("CNCT ");
			else if (cl.state == Defines.cs_zombie)
				clientserver.Com_Printf("ZMBI ");
			else
			{
				ping = cl.ping < 9999 ? cl.ping : 9999;
				clientserver.Com_Printf($"{ping,4} ");
			}

			clientserver.Com_Printf(cl.name);
			l = 16 - cl.name.Length;

			for (j = 0; j < l; j++)
				clientserver.Com_Printf(" ");

			clientserver.Com_Printf($"{SV_INIT.svs.realtime - cl.lastmessage,7} ");
			...
			clientserver.Com_Printf($"{cl.netchan.qport,5}");
	...
	*/
	...
	{
		clientserver.Com_Printf("Server info settings:\n");
		argproc.Info_Print(cvar.Cvar_Serverinfo());
	}
	...
	*/
	public static void SV_DumpUser_f()
	{
		if (cmdparser.Cmd_Argc() != 2)
		{
			clientserver.Com_Printf("Usage: info <userid>\n");
		...
		clientserver.Com_Printf("userinfo\n");
		clientserver.Com_Printf("--------\n");
		argproc.Info_Print(SV_MAIN.sv_client.userinfo);
	}
	...
	*/
	public static void SV_ServerRecord_f()
	{
		...
		if (cmdparser.Cmd_Argc() != 2)
		{
			clientserver.Com_Printf("serverrecord <demoname>\n");
		...
		MSG.WriteString(buf, cvar.Cvar_VariableString("gamedir"));
		...
		clientserver.Com_DPrintf("signon message length: " + buf.cursize + "\n");
	...
	*/
	public static void SV_ServerStop_f()
	{
		if (SV_INIT.svs.demofile == null)
		{
			clientserver.Com_Printf("Not doing a serverrecord.\n");
		...
		clientserver.Com_Printf("Recording completed.\n");
	}
	...
	*/
	...
	{
		cmdparser.Cmd_AddCommand("heartbeat", () => { SV_CCMDS.SV_Heartbeat_f(); });

		cmdparser.Cmd_AddCommand("kick", () => { SV_CCMDS.SV_Kick_f(); });

		cmdparser.Cmd_AddCommand("status", () => { SV_CCMDS.SV_Status_f(); });

		cmdparser.Cmd_AddCommand("serverinfo", () => { SV_CCMDS.SV_Serverinfo_f(); });

		cmdparser.Cmd_AddCommand("dumpuser", () => { SV_CCMDS.SV_DumpUser_f(); });

		cmdparser.Cmd_AddCommand("map", () => { SV_CCMDS.SV_Map_f(); });

		cmdparser.Cmd_AddCommand("demomap", () => { SV_CCMDS.SV_DemoMap_f(); });

		cmdparser.Cmd_AddCommand("gamemap", () => { SV_CCMDS.SV_GameMap_f(); });

		cmdparser.Cmd_AddCommand("setmaster", () => { SV_CCMDS.SV_SetMaster_f(); });

		if (frame.dedicated.value != 0)
			cmdparser.Cmd_AddCommand("say", () => { SV_CCMDS.SV_ConSay_f(); });

		cmdparser.Cmd_AddCommand("serverrecord", () => { SV_CCMDS.SV_ServerRecord_f(); });

		cmdparser.Cmd_AddCommand("serverstop", () => { SV_CCMDS.SV_ServerStop_f(); });

		cmdparser.Cmd_AddCommand("save", () => { SV_CCMDS.SV_Savegame_f(); });

		cmdparser.Cmd_AddCommand("load", () => { SV_CCMDS.SV_Loadgame_f(); });

		cmdparser.Cmd_AddCommand("killserver", () => { SV_CCMDS.SV_KillServer_f(); });

		cmdparser.Cmd_AddCommand("sv", () => { SV_CCMDS.SV_ServerCommand_f(); });
	}
}
