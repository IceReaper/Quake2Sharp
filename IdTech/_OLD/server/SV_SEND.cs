	...
	*/
	public static void SV_Multicast(float[] origin, int to)
	{
		...
		switch (to)
		{
			...
			default:
				mask = null;
				clientserver.Com_Error(Defines.ERR_FATAL, "SV_Multicast: bad to:" + to + "\n");
	...
	*/
	public static void SV_StartSound(float[] origin, edict_t entity, int channel, int soundindex, float volume, float attenuation, float timeofs)
	{
		...
		if (volume < 0 || volume > 1.0)
			clientserver.Com_Error(Defines.ERR_FATAL, "SV_StartSound: volume = " + volume);

		if (attenuation < 0 || attenuation > 4)
			clientserver.Com_Error(Defines.ERR_FATAL, "SV_StartSound: attenuation = " + attenuation);

		//	if (channel < 0 || channel > 15)
		//		Com_Error (ERR_FATAL, "SV_StartSound: channel = %i", channel);

		if (timeofs < 0 || timeofs > 0.255)
			clientserver.Com_Error(Defines.ERR_FATAL, "SV_StartSound: timeofs = " + timeofs);
	...
	*/
	public static bool SV_SendClientDatagram(client_t client)
	{
		...
		if (client.datagram.overflowed)
			clientserver.Com_Printf("WARNING: datagram overflowed for " + client.name + "\n");
		else
		...
		if (SV_SEND.msg.overflowed)
		{
			...
			clientserver.Com_Printf("WARNING: msg overflowed for " + client.name + "\n");
	...
	*/
	public static void SV_DemoCompleted()
	{
		if (SV_INIT.sv.demofile != null)
		{
			...
			{
				clientserver.Com_Printf("IOError closing demo file:" + e);
			}
	...
	public static void SV_SendClientMessages()
	{
		...
		if (SV_INIT.sv.state == Defines.ss_demo && SV_INIT.sv.demofile != null)
		{
			...
			else
			{
				...
				if (msglen > Defines.MAX_MSGLEN)
					clientserver.Com_Error(Defines.ERR_DROP, "SV_SendClientMessages: msglen > MAX_MSGLEN");
				...
				{
					clientserver.Com_Printf("IOError: reading demo file, " + e1);
				}
		...
		for (i = 0; i < SV_MAIN.maxclients.value; i++)
		{
			...
			else
			{
				...
				if (c.netchan.message.cursize != 0 || frame.curtime - c.netchan.last_sent > 1000)
