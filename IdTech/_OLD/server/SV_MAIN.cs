...
public class SV_MAIN
{
	...
	{
		clientserver.Com_Printf("Ping acknowledge from " + NET.AdrToString(Globals.net_from) + "\n");
	}
	...
	public static void SVC_Info()
	{
		...
		version = Lib.atoi(cmdparser.Cmd_Argv(1));
	...
	public static void SVC_GetChallenge()
	{
		...
		if (i == Defines.MAX_CHALLENGES)
		{
			...
			SV_INIT.svs.challenges[oldest].time = (int)frame.curtime;
			i = oldest;
		}
	...
	public static void SVC_DirectConnect()
	{
		...
		clientserver.Com_DPrintf("SVC_DirectConnect ()\n");

		version = Lib.atoi(cmdparser.Cmd_Argv(1));

		if (version != Defines.PROTOCOL_VERSION)
		{
			Netchan.OutOfBandPrint(Defines.NS_SERVER, adr, "print\nServer is version " + Globals.YQ2VERSION + "\n");
			clientserver.Com_DPrintf("    rejected connect from version " + version + "\n");

			return;
		}

		qport = Lib.atoi(cmdparser.Cmd_Argv(2));
		var challenge = Lib.atoi(cmdparser.Cmd_Argv(3));
		userinfo = cmdparser.Cmd_Argv(4);
		...
		if (SV_INIT.sv.attractloop)
		{
			...
			{
				clientserver.Com_Printf("Remote connect in attract loop.  Ignored.\n");
				Netchan.OutOfBandPrint(Defines.NS_SERVER, adr, "print\nConnection refused.\n");
		...
		for (i = 0; i < SV_MAIN.maxclients.value; i++)
		{
			...
			if (NET.CompareBaseAdr(adr, cl.netchan.remote_address) && (cl.netchan.qport == qport || adr.port == cl.netchan.remote_address.port))
			{
				...
				{
					clientserver.Com_DPrintf(NET.AdrToString(adr) + ":reconnect rejected : too soon\n");

					return;
				}

				clientserver.Com_Printf(NET.AdrToString(adr) + ":reconnect\n");
		...
		{
			Netchan.OutOfBandPrint(Defines.NS_SERVER, adr, "print\nServer is full.\n");
			clientserver.Com_DPrintf("Rejected a connection.\n");
	...
	public static void gotnewcl(int i, int challenge, string userinfo, netadr_t adr, int qport)
	{
		...
		if (!PlayerClient.ClientConnect(ent, userinfo))
		{
			...
			clientserver.Com_DPrintf("Game rejected a connection.\n");
		...
		clientserver.Com_DPrintf("new client added.\n");
	}
	...
	public static int Rcon_Validate()
	{
		...
		if (0 != Lib.strcmp(cmdparser.Cmd_Argv(1), SV_MAIN.rcon_password.@string))
			return 0;
	...
	public static void SVC_RemoteCommand()
	{
		...
		if (i == 0)
			clientserver.Com_Printf("Bad rcon from " + NET.AdrToString(Globals.net_from) + ":\n" + msg + "\n");
		else
			clientserver.Com_Printf("Rcon from " + NET.AdrToString(Globals.net_from) + ":\n" + msg + "\n");

		clientserver.Com_BeginRedirect(
			Defines.RD_PACKET,
			(target, buffer) => { SV_SEND.SV_FlushRedirect(target, Lib.stringToBytes(buffer.ToString())); }
		);

		if (0 == SV_MAIN.Rcon_Validate())
			clientserver.Com_Printf("Bad rcon_password.\n");
		else
		{
			remaining = "";

			for (i = 2; i < cmdparser.Cmd_Argc(); i++)
			{
				remaining += cmdparser.Cmd_Argv(i);
				remaining += " ";
			}

			cmdparser.Cmd_ExecuteString(remaining);
		}

		clientserver.Com_EndRedirect();
	}
	...
	public static void SV_ConnectionlessPacket()
	{
		...
		//clientserver.Com_Printf("Packet " + NET.AdrToString(Netchan.net_from) + " : " + c + "\n");
		//clientserver.Com_Printf(Lib.hexDump(net_message.data, 64, false) + "\n");
		...
		{
			clientserver.Com_Printf("bad connectionless packet from " + NET.AdrToString(Globals.net_from) + "\n");
			clientserver.Com_Printf("[" + s + "]\n");
			clientserver.Com_Printf("" + Lib.hexDump(Globals.net_message.data, 128, false));
		}
	}
	...
	public static void SV_ReadPackets()
	{
		...
		while (NET.GetPacket(Defines.NS_SERVER, Globals.net_from, Globals.net_message))
		{
			...
			for (i = 0; i < SV_MAIN.maxclients.value; i++)
			{
				...
				{
					clientserver.Com_Printf("SV_ReadPackets: fixing up a translated port\n");
		...
		if (0 == SV_MAIN.sv_paused.value || SV_MAIN.maxclients.value > 1)
		{
			...
			if (SV_INIT.sv.time < SV_INIT.svs.realtime)
			{
				if (SV_MAIN.sv_showclamp.value != 0)
					clientserver.Com_Printf("sv highclamp\n");
	...
	{
		frame.time_before_game = frame.time_after_game = 0;
		...
		if (0 == SV_MAIN.sv_timedemo.value && SV_INIT.svs.realtime < SV_INIT.sv.time)
		{
			...
			{
				if (SV_MAIN.sv_showclamp.value != 0)
					clientserver.Com_Printf("sv lowclamp\n");
	...
	public static void Master_Heartbeat()
	{
		...
		if (frame.dedicated == null || 0 == frame.dedicated.value)
		...
		for (i = 0; i < Defines.MAX_MASTERS; i++)
		{
			...
			{
				clientserver.Com_Printf("Sending heartbeat to " + NET.AdrToString(SV_MAIN.master_adr[i]) + "\n");
	...
	public static void Master_Shutdown()
	{
		...
		// pgm post3.19 change, cvar pointer not validated before dereferencing
		if (null == frame.dedicated || 0 == frame.dedicated.value)
			return; // only dedicated servers send heartbeats
	...
	public static void SV_Init()
	{
		...
		SV_MAIN.rcon_password = cvar.Cvar_Get("rcon_password", "", 0);
		cvar.Cvar_Get("skill", "1", 0);
		cvar.Cvar_Get("deathmatch", "0", Defines.CVAR_LATCH);
		cvar.Cvar_Get("coop", "0", Defines.CVAR_LATCH);
		cvar.Cvar_Get("dmflags", "" + Defines.DF_INSTANT_ITEMS, Defines.CVAR_SERVERINFO);
		cvar.Cvar_Get("fraglimit", "0", Defines.CVAR_SERVERINFO);
		cvar.Cvar_Get("timelimit", "0", Defines.CVAR_SERVERINFO);
		cvar.Cvar_Get("cheats", "0", Defines.CVAR_SERVERINFO | Defines.CVAR_LATCH);
		cvar.Cvar_Get("protocol", "" + Defines.PROTOCOL_VERSION, Defines.CVAR_SERVERINFO | Defines.CVAR_NOSET);

		SV_MAIN.maxclients = cvar.Cvar_Get("maxclients", "1", Defines.CVAR_SERVERINFO | Defines.CVAR_LATCH);
		SV_MAIN.hostname = cvar.Cvar_Get("hostname", "noname", Defines.CVAR_SERVERINFO | Defines.CVAR_ARCHIVE);
		SV_MAIN.timeout = cvar.Cvar_Get("timeout", "125", 0);
		SV_MAIN.zombietime = cvar.Cvar_Get("zombietime", "2", 0);
		SV_MAIN.sv_showclamp = cvar.Cvar_Get("showclamp", "0", 0);
		SV_MAIN.sv_paused = cvar.Cvar_Get("paused", "0", 0);
		SV_MAIN.sv_timedemo = cvar.Cvar_Get("timedemo", "0", 0);
		SV_MAIN.sv_enforcetime = cvar.Cvar_Get("sv_enforcetime", "0", 0);

		SV_MAIN.allow_download = cvar.Cvar_Get("allow_download", "1", Defines.CVAR_ARCHIVE);
		SV_MAIN.allow_download_players = cvar.Cvar_Get("allow_download_players", "0", Defines.CVAR_ARCHIVE);
		SV_MAIN.allow_download_models = cvar.Cvar_Get("allow_download_models", "1", Defines.CVAR_ARCHIVE);
		SV_MAIN.allow_download_sounds = cvar.Cvar_Get("allow_download_sounds", "1", Defines.CVAR_ARCHIVE);
		SV_MAIN.allow_download_maps = cvar.Cvar_Get("allow_download_maps", "1", Defines.CVAR_ARCHIVE);

		SV_MAIN.sv_noreload = cvar.Cvar_Get("sv_noreload", "0", 0);
		SV_MAIN.sv_airaccelerate = cvar.Cvar_Get("sv_airaccelerate", "0", Defines.CVAR_LATCH);
		SV_MAIN.public_server = cvar.Cvar_Get("public", "0", 0);
		SV_MAIN.sv_reconnect_limit = cvar.Cvar_Get("sv_reconnect_limit", "3", Defines.CVAR_ARCHIVE);
	...
	public static void SV_Shutdown(string finalmsg, bool reconnect)
	{
		...
		clientserver.Com_SetServerState(SV_INIT.sv.state);
