...
public class SV_USER
{
	...
	public static void SV_BeginDemoserver()
	{
		...
		try
		{
			SV_INIT.sv.demofile = new BinaryReader(filesystem.FS_FOpenFile(name, false));
		}
		catch (Exception)
		{
			clientserver.Com_Error(Defines.ERR_DROP, "Couldn't open " + name + "\n");
		}
	...
	public static void SV_New_f()
	{
		...
		clientserver.Com_DPrintf("New() from " + SV_MAIN.sv_client.name + "\n");

		if (SV_MAIN.sv_client.state != Defines.cs_connected)
		{
			clientserver.Com_Printf("New not valid -- already spawned\n");
	...
	public static void SV_Configstrings_f()
	{
		...
		clientserver.Com_DPrintf("Configstrings() from " + SV_MAIN.sv_client.name + "\n");

		if (SV_MAIN.sv_client.state != Defines.cs_connected)
		{
			clientserver.Com_Printf("configstrings not valid -- already spawned\n");

			return;
		}

		// handle the case of a level changing while a client was connecting
		if (Lib.atoi(cmdparser.Cmd_Argv(1)) != SV_INIT.svs.spawncount)
		{
			clientserver.Com_Printf("SV_Configstrings_f from different level\n");
			SV_USER.SV_New_f();

			return;
		}

		start = Lib.atoi(cmdparser.Cmd_Argv(2));
	...
	public static void SV_Baselines_f()
	{
		...
		clientserver.Com_DPrintf("Baselines() from " + SV_MAIN.sv_client.name + "\n");

		if (SV_MAIN.sv_client.state != Defines.cs_connected)
		{
			clientserver.Com_Printf("baselines not valid -- already spawned\n");
		...
		if (Lib.atoi(cmdparser.Cmd_Argv(1)) != SV_INIT.svs.spawncount)
		{
			clientserver.Com_Printf("SV_Baselines_f from different level\n");
			SV_USER.SV_New_f();
	...
	{
		clientserver.Com_DPrintf("Begin() from " + SV_MAIN.sv_client.name + "\n");

		// handle the case of a level changing while a client was connecting
		if (Lib.atoi(cmdparser.Cmd_Argv(1)) != SV_INIT.svs.spawncount)
		{
			clientserver.Com_Printf("SV_Begin_f from different level\n");
			SV_USER.SV_New_f();
		...
		cmdparser.Cbuf_InsertFromDefer();
	}
	...
	public static void SV_BeginDownload_f()
	{
		...
		if (cmdparser.Cmd_Argc() > 2)
			offset = Lib.atoi(cmdparser.Cmd_Argv(2)); // downloaded offset
		...
		SV_MAIN.sv_client.download = filesystem.FS_LoadFile(name);
		...
		if (SV_MAIN.sv_client.download == null // special check for maps, if it
		    ...
		    // allow
		    // download ZOID
		    || (name.StartsWith("maps/") && filesystem.file_from_protected_pack))
		{
			clientserver.Com_DPrintf("Couldn't download " + name + " to " + SV_MAIN.sv_client.name + "\n");
		...
		clientserver.Com_DPrintf("Downloading " + name + " to " + SV_MAIN.sv_client.name + "\n");
	}
	...
	public static void SV_Nextserver()
	{
		...
		if (SV_INIT.sv.state == Defines.ss_game || (SV_INIT.sv.state == Defines.ss_pic && 0 == cvar.Cvar_VariableValue("coop")))
	...
	public static void SV_Nextserver_f()
	{
		if (Lib.atoi(cmdparser.Cmd_Argv(1)) != SV_INIT.svs.spawncount)
		{
			clientserver.Com_DPrintf("Nextserver() from wrong level, from " + SV_MAIN.sv_client.name + "\n");

			return; // leftover from last server
		}

		clientserver.Com_DPrintf("Nextserver() from " + SV_MAIN.sv_client.name + "\n");
	...
	public static void SV_ExecuteUserCommand(string s)
	{
		...
		cmdparser.Cmd_TokenizeString(s, true);
		...
		for (; i < SV_USER.ucmds.Length; i++)
		{
			...
			if (cmdparser.Cmd_Argv(0).Equals(u.name))
			{
				u.r();
	...
	public static void SV_ClientThink(client_t cl, usercmd_t cmd)
	{
		...
		{
			clientserver.Com_DPrintf("commandMsec underflow from " + cl.name + "\n");
	...
	public static void SV_ExecuteClientMessage(client_t cl)
	{
		...
		while (true)
		{
			...
			{
				clientserver.Com_Printf("SV_ReadClientMessage: bad read:\n");
				clientserver.Com_Printf(Lib.hexDump(Globals.net_message.data, 32, false));
			...
			{
				default:
					clientserver.Com_Printf("SV_ReadClientMessage: unknown command char\n");
					SV_MAIN.SV_DropClient(cl);
				...
				case Defines.clc_move:
					...
					if (0 == SV_MAIN.sv_paused.value)
					{
						...
						if (net_drop < 20)
						{
							...
							//	clientserver.Com_Printf ("drop %i\n", net_drop);
							while (net_drop > 2)
							{
