...
public class Info
{
	...
	public static string Info_SetValueForKey(string s, string key, string value)
	{
		...
		{
			clientserver.Com_Printf("Can't use keys or values with a \\\n");
		...
		if (key.IndexOf(';') != -1)
		{
			clientserver.Com_Printf("Can't use keys or values with a semicolon\n");
		...
		{
			clientserver.Com_Printf("Keys and values must be < 64 characters.\n");
		...
		{
			clientserver.Com_Printf("Info string length exceeded\n");
	...
	public static string Info_RemoveKey(string s, string key)
	{
		...
		if (key.IndexOf('\\') != -1)
		{
			clientserver.Com_Printf("Can't use a key with a \\\n");
