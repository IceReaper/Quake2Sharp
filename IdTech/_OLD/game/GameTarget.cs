...
public class GameTarget
{
     ...
     */
	public static EntUseAdapter Use_Target_Tent = new("Use_Target_Tent", (ent, other, activator) =>
	{
     ...
     */
	public static EntUseAdapter Use_Target_Speaker = new("Use_Target_Speaker", (ent, other, activator) =>
	{
		int chan;
	...
	public static EntUseAdapter Use_Target_Help = new("Use_Target_Help", (ent, other, activator) =>
	{
		if ((ent.spawnflags & 1) != 0)
     ...
     */
	private static readonly EntUseAdapter use_target_secret = new("use_target_secret", (ent, other, activator) =>
	{
     ...
     * These are single use targets.
     */
	private static readonly EntUseAdapter use_target_goal = new("use_target_goal", (ent, other, activator) =>
	{
		GameBase.gi.sound(ent, Defines.CHAN_VOICE, ent.noise_index, 1, Defines.ATTN_NORM, 0);
     ...
     */
	private static readonly EntThinkAdapter target_explosion_explode = new("target_explosion_explode", self =>
	{
		float save;
	...
	private static readonly EntUseAdapter use_target_explosion = new("use_target_explosion", (self, other, activator) =>
	{
		self.activator = activator;

		if (0 == self.delay)
		{
			GameTarget.target_explosion_explode.think(self);

			return;
		}

		self.think = GameTarget.target_explosion_explode;
		self.nextthink = GameBase.level.time + self.delay;
	});
     ...
     * "map" when fired
     */
	private static readonly EntUseAdapter use_target_changelevel = new("use_target_changelevel", (self, other, activator) =>
	{
     ...
     */
	private static readonly EntUseAdapter use_target_splash = new("use_target_splash", (self, other, activator) =>
	{
		GameBase.gi.WriteByte(Defines.svc_temp_entity);
     ...
     */
	private static readonly EntUseAdapter use_target_spawner = new("use_target_spawner", (self, other, activator) =>
	{
		edict_t ent;
     ...
     */
	public static EntUseAdapter trigger_crosslevel_trigger_use = new("trigger_crosslevel_trigger_use", (self, other, activator) =>
	{
     ...
     * (default 1)
     */
	private static readonly EntThinkAdapter target_crosslevel_target_think = new("target_crosslevel_target_think", self =>
	{
     ...
     */
	public static EntThinkAdapter target_laser_think = new("target_laser_think", self =>
	{
		edict_t ignore;
		float[] start = { 0, 0, 0 };
		float[] end = { 0, 0, 0 };
		...
		while (true)
		{
			...
			if (0 == (tr.ent.svflags & Defines.SVF_MONSTER) && null == tr.ent.client)
			{
				if ((self.spawnflags & 0x80000000) != 0)
				{
					self.spawnflags &= (int)~0x80000000;
					GameBase.gi.WriteByte(Defines.svc_temp_entity);
	...
	public static EntUseAdapter target_laser_use = new("target_laser_use", (self, other, activator) =>
	{
		self.activator = activator;
	...
	private static readonly EntThinkAdapter target_laser_start = new("target_laser_start", self =>
	{
		...
		if (null == self.enemy)
		{
			if (self.target != null)
			{
				var edit = GameBase.G_Find(null, GameBase.findByTarget, self.target);
		...
		self.think = GameTarget.target_laser_think;
     ...
     */
	private static readonly EntThinkAdapter target_lightramp_think = new("target_lightramp_think", self =>
	{
		char[] tmp = { (char)('a' + (int)(self.movedir[0] + (GameBase.level.time - self.timestamp) / Defines.FRAMETIME * self.movedir[2])) };

		GameBase.gi.configstring(Defines.CS_LIGHTS + self.enemy.style, new(tmp));
	...
	private static readonly EntUseAdapter target_lightramp_use = new("target_lightramp_use", (self, other, activator) =>
	{
		if (self.enemy == null)
		{
			edict_t e;
		...
		GameTarget.target_lightramp_think.think(self);
	});
     ...
     * (default:5)
     */
	private static readonly EntThinkAdapter target_earthquake_think = new("target_earthquake_think", self =>
	{
		int i;
		edict_t e;
	...
	private static readonly EntUseAdapter target_earthquake_use = new("target_earthquake_use", (self, other, activator) =>
	{
