...
public class GameUtil
{
	public static void checkClassname(edict_t ent)
	{
		if (ent.classname == null)
			clientserver.Com_Printf("edict with classname = null: " + ent.index);
	}
	...
	public static void G_FreeEdict(edict_t ed)
	{
		...
		GameBase.g_edicts[ed.index] = new(ed.index);
		ed.classname = "freed";
	...
	public static bool OnSameTeam(edict_t ent1, edict_t ent2)
	{
		...
		if (GameUtil.ClientTeam(ent1).Equals(GameUtil.ClientTeam(ent2)))
			return true;
	...
	public static bool FindTarget(edict_t self)
	{
		...
		if (!heardit)
		{
			...
			{
				if (client.show_hostile < GameBase.level.time && !GameUtil.infront(self, client))
					return false;
			}
			else if (r == Defines.RANGE_MID)
			{
				if (!GameUtil.infront(self, client))
					return false;
			}
	...
	public static EntThinkAdapter Think_Delay = new("Think_Delay", ent =>
	{
	...
	public static EntThinkAdapter G_FreeEdictA = new("G_FreeEdictA", ent =>
	{
		GameUtil.G_FreeEdict(ent);
	...
	public static EntThinkAdapter MegaHealth_think = new("MegaHealth_think", self =>
	{
	...
	public static EntThinkAdapter M_CheckAttack = new("M_CheckAttack", self =>
	{
		float[] spot1 = { 0, 0, 0 };
	...
	public static EntUseAdapter monster_use = new("monster_use", (self, other, activator) =>
	{
		if (self.enemy != null)
			return;
