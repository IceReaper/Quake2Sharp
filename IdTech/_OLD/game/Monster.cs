...
public class Monster
{
	...
	public static void monster_start_go(edict_t self)
	{
		...
		if (self.target != null)
		{
			...
			fixup = false;
			/*
			 * if (true) { clientserver.Com_Printf("all entities:\n");
			 *
			 * for (int n = 0; n < Game.globals.num_edicts; n++) { edict_t ent =
			 * GameBase.g_edicts[n]; clientserver.Com_Printf( "|%4i | %25s
	...
	public static EntThinkAdapter monster_think = new("monster_think", self =>
	{
		M.M_MoveFrame(self);
	...
	public static EntThinkAdapter monster_triggered_spawn = new("monster_trigger_spawn", self =>
	{
		self.s.origin[2] += 1;
	...
	// us
	public static EntUseAdapter monster_triggered_spawn_use = new("monster_trigger_spawn_use", (self, other, activator) =>
	{
		self.think = Monster.monster_triggered_spawn;
		self.nextthink = GameBase.level.time + Defines.FRAMETIME;
	...
	public static EntThinkAdapter monster_triggered_start = new("monster_triggered_start", self =>
	{
		if (self.index == 312)
			clientserver.Com_Printf("monster_triggered_start\n");

		self.solid = Defines.SOLID_NOT;
		self.movetype = Defines.MOVETYPE_NONE;
		self.svflags |= Defines.SVF_NOCLIENT;
		self.nextthink = 0;
		self.use = Monster.monster_triggered_spawn_use;
