...
public class VID
{
	...
	*/
	private static bool LoadRefresh(string name)
	{
		...
		clientserver.Com_Printf("------- Loading " + name + " -------\n");
		var found = false;
		...
		if (!found)
		{
			clientserver.Com_Printf("LoadLibrary(\"" + name + "\") failed\n");

			return false;
		}

		clientserver.Com_Printf("LoadLibrary(\"" + name + "\")\n");
		Globals.re = Renderer.getDriver(name);

		if (Globals.re == null)
			clientserver.Com_Error(Defines.ERR_FATAL, name + " can't load but registered");

		if (Globals.re.apiVersion() != Defines.API_VERSION)
		{
			VID.FreeReflib();
			clientserver.Com_Error(Defines.ERR_FATAL, name + " has incompatible api_version");
		}
			...
			*/
			...
			if (!VID.LoadRefresh(VID.vid_ref.@string))
			{
				...
				if (VID.vid_ref.@string.Equals(Renderer.getDefaultName()))
				{
					...
					clientserver.Com_Printf("Refresh failed\n");
					VID.gl_mode = cvar.Cvar_Get("r_mode", "0", 0);

					if (VID.gl_mode.value != 0.0f)
					{
						clientserver.Com_Printf("Trying mode 0\n");
						cvar.Cvar_SetValue("r_mode", 0);

						if (!VID.LoadRefresh(VID.vid_ref.@string))
							clientserver.Com_Error(Defines.ERR_FATAL, "Couldn't fall back to " + renderer + " refresh!");
					}
					else
						clientserver.Com_Error(Defines.ERR_FATAL, "Couldn't fall back to " + renderer + " refresh!");
				}

				cvar.Cvar_Set("vid_ref", renderer);
	...
	*/
	public static void Init()
	{
		...
		VID.vid_ref = cvar.Cvar_Get("vid_ref", Renderer.getPreferedName(), Defines.CVAR_ARCHIVE);
		VID.vid_xpos = cvar.Cvar_Get("vid_xpos", "3", Defines.CVAR_ARCHIVE);
		VID.vid_ypos = cvar.Cvar_Get("vid_ypos", "22", Defines.CVAR_ARCHIVE);
		VID.vid_width = cvar.Cvar_Get("vid_width", "640", Defines.CVAR_ARCHIVE);
		VID.vid_height = cvar.Cvar_Get("vid_height", "480", Defines.CVAR_ARCHIVE);
		VID.vid_fullscreen = cvar.Cvar_Get("vid_fullscreen", "0", Defines.CVAR_ARCHIVE);
		VID.vid_gamma = cvar.Cvar_Get("vid_gamma", "1", Defines.CVAR_ARCHIVE);
		VID.vid_modes[11].width = (int)VID.vid_width.value;
		VID.vid_modes[11].height = (int)VID.vid_height.value;
		/* Add some console commands that we want to handle */
		cmdparser.Cmd_AddCommand("vid_restart", () => { VID.Restart_f(); });
	...
	*/
	...
	private static void ScreenSizeCallback(object s)
	{
		...
		cvar.Cvar_SetValue("viewsize", slider.curvalue * 10);
	}
	...
	private static void BrightnessCallback(object s)
	{
		    ...
		    || VID.vid_ref.@string.Equals("softx", StringComparison.OrdinalIgnoreCase))
		{
			...
			cvar.Cvar_SetValue("vid_gamma", gamma);
		}
	}
	...
	private static void ApplyChanges(object unused)
	{
		...
		*/
		...
		cvar.Cvar_SetValue("vid_gamma", gamma);
		cvar.Cvar_SetValue("r_modulate", modulate);
		cvar.Cvar_SetValue("gl1_picmip", 3 - VID.s_tq_slider.curvalue);
		cvar.Cvar_SetValue("vid_fullscreen", VID.s_fs_box.curvalue);
		cvar.Cvar_SetValue("r_vsync", (int)VID.s_vsync_box.curvalue);

		// set always true because of vid_ref or mode changes
		VID.gl_swapinterval.modified = true;
		cvar.Cvar_SetValue("gl_ext_palettedtexture", VID.s_paletted_texture_box.curvalue);
		cvar.Cvar_SetValue("r_mode", VID.s_mode_list.curvalue);
		cvar.Cvar_Set("vid_ref", VID.drivers[VID.s_ref_list.curvalue]);
		cvar.Cvar_Set("gl_driver", VID.drivers[VID.s_ref_list.curvalue]);
	...
	*/
	public static void MenuInit()
	{
		...
		if (VID.gl_driver == null)
			VID.gl_driver = cvar.Cvar_Get("gl_driver", Renderer.getPreferedName(), 0);

		if (VID.gl_picmip == null)
			VID.gl_picmip = cvar.Cvar_Get("gl1_picmip", "0", 0);

		if (VID.gl_mode == null)
			VID.gl_mode = cvar.Cvar_Get("r_mode", "3", 0);

		if (VID.gl_ext_palettedtexture == null)
			VID.gl_ext_palettedtexture = cvar.Cvar_Get("gl_ext_palettedtexture", "1", Defines.CVAR_ARCHIVE);

		if (VID.gl_swapinterval == null)
			VID.gl_swapinterval = cvar.Cvar_Get("r_vsync", "0", Defines.CVAR_ARCHIVE);
