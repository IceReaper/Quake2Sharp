...
public class Key
{
	...
	public static void Init()
	{
		...
		//
		cmdparser.Cmd_AddCommand("bind", Key.Bind_f);
		cmdparser.Cmd_AddCommand("unbind", Key.Unbind_f);
		cmdparser.Cmd_AddCommand("unbindall", Key.Unbindall_f);
		cmdparser.Cmd_AddCommand("bindlist", Key.Bindlist_f);
	}
	...
	public static void Event(int key, bool down, int time, string? text)
	{
		...
		if (down)
		{
			...
			if (key >= 200 && Globals.keybindings[key] == null)
				clientserver.Com_Printf(Key.KeynumToString(key) + " is unbound, hit F4 to set.\n");
		}
		else
		...
		if (key == Key.K_ESCAPE)
		{
			...
			if (Globals.cl.frame.playerstate.stats[Defines.STAT_LAYOUTS] != 0 && Globals.cls.key_dest == Defines.key_game)
			{
				...
				cmdparser.Cbuf_AddText("cmd putaway\n");
			...
			switch (Globals.cls.key_dest)
			{
				...
				default:
					clientserver.Com_Error(Defines.ERR_FATAL, "Bad cls.key_dest");
	...
	public static void Message(int key, string? text)
	{
		if (key == Key.K_ENTER || key == Key.K_KP_ENTER)
		{
			if (Globals.chat_team)
				cmdparser.Cbuf_AddText("say_team \"");
			else
				cmdparser.Cbuf_AddText("say \"");

			cmdparser.Cbuf_AddText(Globals.chat_buffer);
	...
	public static void Console(int key, string? text)
	{
		if (key == Key.K_ENTER || key == Key.K_KP_ENTER)
		{
			...
			if (Globals.key_lines[Globals.edit_line][1] == '\\' || Globals.key_lines[Globals.edit_line][1] == '/')
				cmdparser.Cbuf_AddText(new string(Globals.key_lines[Globals.edit_line], 2, Lib.strlen(Globals.key_lines[Globals.edit_line]) - 2));
			else
				cmdparser.Cbuf_AddText(new string(Globals.key_lines[Globals.edit_line], 1, Lib.strlen(Globals.key_lines[Globals.edit_line]) - 1));

			cmdparser.Cbuf_AddText("\n");
			clientserver.Com_Printf(new string(Globals.key_lines[Globals.edit_line], 0, Lib.strlen(Globals.key_lines[Globals.edit_line])) + "\n");
	...
	{
		clientserver.Com_Printf(type);

		for (var i = 0; i < compl.Count; i++)
			clientserver.Com_Printf((string)compl[i] + " ");
	...
	private static void CompleteCommand()
	{
		...
		var cmds = new List<string>{ cmdparser.Cmd_CompleteCommand(s) };
		var c = cmds.Count;
	...
	{
		var c = cmdparser.Cmd_Argc();

		if (c < 2)
		{
			clientserver.Com_Printf("bind <key> [command] : attach a command to a key\n");

			return;
		}

		var b = Key.StringToKeynum(cmdparser.Cmd_Argv(1));

		if (b == -1)
		{
			clientserver.Com_Printf("\"" + cmdparser.Cmd_Argv(1) + "\" isn't a valid key\n");
		...
		if (c == 2)
		{
			if (Globals.keybindings[b] != null)
				clientserver.Com_Printf("\"" + cmdparser.Cmd_Argv(1) + "\" = \"" + Globals.keybindings[b] + "\"\n");
			else
				clientserver.Com_Printf("\"" + cmdparser.Cmd_Argv(1) + "\" is not bound\n");
	...
	private static void Key_Unbind_f()
	{
		...
		{
			clientserver.Com_Printf("unbind <key> : remove commands from a key\n");
	...
	private static void Key_Bindlist_f()
	{
		for (var i = 0; i < 256; i++)
		{
			if (Globals.keybindings[i] != null && Globals.keybindings[i].Length != 0)
				clientserver.Com_Printf(Key.KeynumToString(i) + " \"" + Globals.keybindings[i] + "\"\n");
		}
	}
