...
public class CL_ents
{
	...
	public static void ParsePacketEntities(frame_t oldframe, frame_t newframe)
	{
		...
		while (true)
		{
			...
			if (newnum >= Defines.MAX_EDICTS)
				clientserver.Com_Error(Defines.ERR_DROP, "CL_ParsePacketEntities: bad number:" + newnum);

			if (Globals.net_message.readcount > Globals.net_message.cursize)
				clientserver.Com_Error(Defines.ERR_DROP, "CL_ParsePacketEntities: end of message");
			...
			while (oldnum < newnum)
			{
				...
				if (Globals.cl_shownet.value == 3)
					clientserver.Com_Printf("   unchanged: " + oldnum + "\n");
			...
			if ((bits & Defines.U_REMOVE) != 0)
			{
				...
				// current frame
				if (Globals.cl_shownet.value == 3)
					clientserver.Com_Printf("   remove: " + newnum + "\n");

				if (oldnum != newnum)
					clientserver.Com_Printf("U_REMOVE: oldnum != newnum\n");
			...
			if (oldnum == newnum)
			{
				...
				if (Globals.cl_shownet.value == 3)
					clientserver.Com_Printf("   delta: " + newnum + "\n");
			...
			if (oldnum > newnum)
			{
				// delta from baseline
				if (Globals.cl_shownet.value == 3)
					clientserver.Com_Printf("   baseline: " + newnum + "\n");
	...
	public static void ParseFrame()
	{
		...
		if (Globals.cl_shownet.value == 3)
			clientserver.Com_Printf("   frame:" + Globals.cl.frame.serverframe + "  delta:" + Globals.cl.frame.deltaframe + "\n");
		...
		else
		{
			...
			{
				// should never happen
				clientserver.Com_Printf("Delta from invalid frame (not supposed to happen!).\n");
			}
			...
			if (old.serverframe != Globals.cl.frame.deltaframe)
			{
				...
				clientserver.Com_Printf("Delta frame too old.\n");
			}
			else if (Globals.cl.parse_entities - old.parse_entities > Defines.MAX_PARSE_ENTITIES - 128)
				clientserver.Com_Printf("Delta parse_entities too old.\n");
			else
		...
		if (cmd != Defines.svc_playerinfo)
			clientserver.Com_Error(Defines.ERR_DROP, "CL_ParseFrame: not playerinfo");
		...
		if (cmd != Defines.svc_packetentities)
			clientserver.Com_Error(Defines.ERR_DROP, "CL_ParseFrame: not packetentities");
	...
	public static void AddEntities()
	{
		...
		if (Globals.cl.time > Globals.cl.frame.servertime)
		{
			if (Globals.cl_showclamp.value != 0)
				clientserver.Com_Printf("high clamp " + (Globals.cl.time - Globals.cl.frame.servertime) + "\n");
		...
		{
			if (Globals.cl_showclamp.value != 0)
				clientserver.Com_Printf("low clamp " + (Globals.cl.frame.servertime - 100 - Globals.cl.time) + "\n");
	...
	public static void GetEntitySoundOrigin(int ent, float[] org)
	{
		...
		if (ent < 0 || ent >= Defines.MAX_EDICTS)
			clientserver.Com_Error(Defines.ERR_DROP, "CL_GetEntitySoundOrigin: bad ent");
