...
public class Menu
{
	public static bool menu_startdemoloop;
	...
	private static void PushMenu(Action draw, Func<int, string?, string> key)
	{
		...
		if (i == Menu.m_menudepth)
		{
			if (Menu.m_menudepth >= Menu.MAX_MENU_DEPTH)
				clientserver.Com_Error(Defines.ERR_FATAL, "PushMenu: MAX_MENU_DEPTH");
	...
	public static void PopMenu()
	{
		...
		if (Menu.m_menudepth < 0)
			clientserver.Com_Error(Defines.ERR_FATAL, "PopMenu: depth < 1");
	...
	{
		cvar.Cvar_SetValue("crosshair", Menu.s_options_crosshair_box.curvalue);
	}

	private static void JoystickFunc(object unused)
	{
		cvar.Cvar_SetValue("in_joystick", Menu.s_options_joystick_box.curvalue);
	}
	...
	{
		cvar.Cvar_SetValue("cl_run", Menu.s_options_alwaysrun_box.curvalue);
	}

	private static void FreeLookFunc(object unused)
	{
		cvar.Cvar_SetValue("freelook", Menu.s_options_freelook_box.curvalue);
	}

	private static void MouseSpeedFunc(object unused)
	{
		cvar.Cvar_SetValue("sensitivity", Menu.s_options_sensitivity_slider.curvalue / 2.0F);
	}

	private static void NoAltTabFunc(object unused)
	{
		cvar.Cvar_SetValue("win_noalttab", Menu.s_options_noalttab_box.curvalue);
	}
	...
	{
		Menu.s_options_sfxvolume_slider.curvalue = cvar.Cvar_VariableValue("s_volume") * 10;
		Menu.s_options_cdvolume_box.curvalue = 1 - (int)cvar.Cvar_VariableValue("cd_nocd");

		//s_options_quality_list.curvalue = 1 - ((int)
		// cvar.Cvar_VariableValue("s_loadas8bit"));
		var s = cvar.Cvar_VariableString("s_impl");
		...
		cvar.Cvar_SetValue("cl_run", Menu.ClampCvar(0, 1, Globals.cl_run.value));
		...
		cvar.Cvar_SetValue("lookspring", Menu.ClampCvar(0, 1, Globals.lookspring.value));
		Menu.s_options_lookspring_box.curvalue = (int)Globals.lookspring.value;
		cvar.Cvar_SetValue("lookstrafe", Menu.ClampCvar(0, 1, Globals.lookstrafe.value));
		Menu.s_options_lookstrafe_box.curvalue = (int)Globals.lookstrafe.value;
		cvar.Cvar_SetValue("freelook", Menu.ClampCvar(0, 1, Globals.freelook.value));
		Menu.s_options_freelook_box.curvalue = (int)Globals.freelook.value;
		cvar.Cvar_SetValue("crosshair", Menu.ClampCvar(0, 3, Globals.crosshair.value));
		Menu.s_options_crosshair_box.curvalue = (int)Globals.crosshair.value;
		cvar.Cvar_SetValue("in_joystick", Menu.ClampCvar(0, 1, Globals.in_joystick.value));
	...
	{
		cvar.Cvar_SetValue("m_pitch", -Globals.m_pitch.value);
	}

	private static void LookspringFunc(object unused)
	{
		cvar.Cvar_SetValue("lookspring", 1 - Globals.lookspring.value);
	}

	private static void LookstrafeFunc(object unused)
	{
		cvar.Cvar_SetValue("lookstrafe", 1 - Globals.lookstrafe.value);
	}

	private static void UpdateVolumeFunc(object unused)
	{
		cvar.Cvar_SetValue("s_volume", Menu.s_options_sfxvolume_slider.curvalue / 10);
	}

	private static void UpdateCDVolumeFunc(object unused)
	{
		cvar.Cvar_SetValue("cd_nocd", 1 - Menu.s_options_cdvolume_box.curvalue);
	}
	...
	private static void ConsoleFunc(object unused)
	{
		...
		{
			cmdparser.Cbuf_AddText("killserver\n");
	...
	private static void UpdateSoundQualityFunc(object unused)
	{
		...
		//            //			cvar.Cvar_SetValue("s_loadas8bit", 0);
		//            driverNotChanged = S.getDriverName().equals("dummy");
		//            cvar.Cvar_Set("s_impl", "dummy");
		//        } else {
		//            //			cvar.Cvar_SetValue("s_khz", 11);
		//            //			cvar.Cvar_SetValue("s_loadas8bit", 1);
		//            driverNotChanged = S.getDriverName().equals("joal");
		//            cvar.Cvar_Set("s_impl", "joal");
		//        }
	...
	private static void Options_MenuInit()
	{
		...
		Menu.win_noalttab = cvar.Cvar_Get("win_noalttab", "0", Defines.CVAR_ARCHIVE);
		/*
	...
	private static void Menu_Credits_f()
	{
		int n;
		var isdeveloper = 0;
		var b = filesystem.FS_LoadFile("credits");
	...
	private static void StartGame()
	{
		...
		cmdparser.Cbuf_AddText("loading ; killserver ; wait ; newgame\n");
		Globals.cls.key_dest = Defines.key_game;
	}
	...
	private static void Create_Savestrings()
	{
		...
		for (i = 0; i < Menu.MAX_SAVEGAMES; i++)
		{
			...
			name = filesystem.FS_Gamedir() + "/save/save" + i + "/server.ssv";
	...
	private static void LoadGameCallback(object self)
	{
		...
		if (Menu.m_savevalid[a.localdata[0]])
			cmdparser.Cbuf_AddText("load save" + a.localdata[0] + "\n");
	...
	{
		var a = (menuaction_s)self;
		cmdparser.Cbuf_AddText("save save" + a.localdata[0] + "\n");
		Menu.ForceMenuOff();
	}
	...
	{
		if (0 == clientserver.Com_ServerState())
	...
	private static void StartServerActionFunc(object self)
	{
		...
		cvar.Cvar_SetValue("maxclients", Menu.ClampCvar(0, maxclients, maxclients));
		cvar.Cvar_SetValue("timelimit", Menu.ClampCvar(0, timelimit, timelimit));
		cvar.Cvar_SetValue("fraglimit", Menu.ClampCvar(0, fraglimit, fraglimit));
		cvar.Cvar_Set("hostname", Menu.s_hostname_field.buffer.ToString());

		//		cvar.Cvar_SetValue ("deathmatch", !s_rules_box.curvalue );
		//		cvar.Cvar_SetValue ("coop", s_rules_box.curvalue );

		//	  PGM
		if (Menu.s_rules_box.curvalue < 2 || FS.Developer_searchpath(2) != 2)
		{
			cvar.Cvar_SetValue("deathmatch", 1 - (int)Menu.s_rules_box.curvalue);
			cvar.Cvar_SetValue("coop", Menu.s_rules_box.curvalue);
		...
		else
		{
			...
			cvar.Cvar_SetValue("gamerules", Menu.s_rules_box.curvalue);
		}
		...
		if (spot != null)
		{
			if (clientserver.Com_ServerState() != 0)
				cmdparser.Cbuf_AddText("disconnect\n");

			cmdparser.Cbuf_AddText("gamemap \"*" + startmap + "$" + spot + "\"\n");
		}
		else
			cmdparser.Cbuf_AddText("map " + startmap + "\n");
	...
	private static void StartServer_MenuInit()
	{
		 ...
		 */
		mapsname = filesystem.FS_Gamedir() + "/maps.lst";
		...
		if (fp == null)
		{
			buffer = filesystem.FS_LoadFile("maps.lst");

			if (buffer == null)

				//if ((length = FS_LoadFile("maps.lst", (Object *) & buffer))
				// == -1)
				clientserver.Com_Error(Defines.ERR_DROP, "couldn't find maps.lst\n");
		}
		else
		{
			try
			...
			{
				clientserver.Com_Error(Defines.ERR_DROP, "couldn't load maps.lst\n");
			}
		}
		...
		if (Menu.nummaps == 0)
			clientserver.Com_Error(Defines.ERR_DROP, "no maps in maps.lst\n");
		...
		Menu.s_timelimit_field.buffer = new(cvar.Cvar_VariableString("timelimit"));
		...
		Menu.s_fraglimit_field.buffer = new(cvar.Cvar_VariableString("fraglimit"));
		/*
		...
		if (cvar.Cvar_VariableValue("maxclients") == 1)
			...
			Menu.s_maxclients_field.buffer = new(cvar.Cvar_VariableString("maxclients"));
		...
		Menu.s_hostname_field.buffer = new(cvar.Cvar_VariableString("hostname"));
	...
	{
		var dmflags = (int)cvar.Cvar_VariableValue("dmflags");
		var y = 0;
	...
	private static void DownloadCallback(object self)
	{
		...
		if (f == Menu.s_allow_download_box)
			cvar.Cvar_SetValue("allow_download", f.curvalue);
		else if (f == Menu.s_allow_download_maps_box)
			cvar.Cvar_SetValue("allow_download_maps", f.curvalue);
		else if (f == Menu.s_allow_download_models_box)
			cvar.Cvar_SetValue("allow_download_models", f.curvalue);
		else if (f == Menu.s_allow_download_players_box)
			cvar.Cvar_SetValue("allow_download_players", f.curvalue);
		else if (f == Menu.s_allow_download_sounds_box)
			cvar.Cvar_SetValue("allow_download_sounds", f.curvalue);
	}
	...
	private static void DownloadOptions_MenuInit()
	{
		...
		Menu.s_allow_download_box.curvalue = cvar.Cvar_VariableValue("allow_download") != 0 ? 1 : 0;
		...
		Menu.s_allow_download_maps_box.curvalue = cvar.Cvar_VariableValue("allow_download_maps") != 0 ? 1 : 0;
		...
		Menu.s_allow_download_players_box.curvalue = cvar.Cvar_VariableValue("allow_download_players") != 0 ? 1 : 0;
		...
		Menu.s_allow_download_models_box.curvalue = cvar.Cvar_VariableValue("allow_download_models") != 0 ? 1 : 0;
		...
		Menu.s_allow_download_sounds_box.curvalue = cvar.Cvar_VariableValue("allow_download_sounds") != 0 ? 1 : 0;
	...
	private static void AddressBook_MenuInit()
	{
		...
		{
			var adr = cvar.Cvar_Get("adr" + i, "", Defines.CVAR_ARCHIVE);
	...
	private static string AddressBook_MenuKey_f(int key, string? text)
	{
		if (key == Key.K_ESCAPE)
		{
			for (var index = 0; index < Defines.NUM_ADDRESSBOOK_ENTRIES; index++)
				cvar.Cvar_Set("adr" + index, Menu.s_addressbook_fields[index].buffer.ToString());
		}
	...
	{
		cvar.Cvar_SetValue("hand", Menu.s_player_handedness_box.curvalue);
	}
	...
	private static void RateCallback(object unused)
	{
		if (Menu.s_player_rate_box.curvalue != Menu.rate_tbl.Length - 1) //sizeof(rate_tbl)
			// / sizeof(*
			// rate_tbl) - 1)
			cvar.Cvar_SetValue("rate", Menu.rate_tbl[Menu.s_player_rate_box.curvalue]);
	}
	...
	private static bool PlayerConfig_ScanDirectories()
	{
		...
		do
		{
			...
			if ((dirnames = filesystem.FS_ListFiles(findname)) != null)
			{
		...
		for (i = 0; i < npms; i++)
		{
			...
			pcxnames = filesystem.FS_ListFiles(scratch);
	...
	private static bool PlayerConfig_MenuInit()
	{
		...
		var hand = cvar.Cvar_Get("hand", "0", Defines.CVAR_USERINFO | Defines.CVAR_ARCHIVE);
		...
		Menu.s_player_handedness_box.curvalue = (int)cvar.Cvar_VariableValue("hand");
		Menu.s_player_handedness_box.itemnames = Menu.handedness;

		for (i = 0; i < Menu.rate_tbl.Length - 1; i++)
		{
			if (cvar.Cvar_VariableValue("rate") == Menu.rate_tbl[i])
				break;
		}
	...
	private static string PlayerConfig_MenuKey(int key, string? text)
	{
		...
		{
			string scratch;
			cvar.Cvar_Set("name", Menu.s_player_name_field.buffer.ToString());

			scratch = Menu.s_pmi[Menu.s_player_model_box.curvalue].directory
				+ "/"
				+ Menu.s_pmi[Menu.s_player_model_box.curvalue].skindisplaynames[Menu.s_player_skin_box.curvalue];

			cvar.Cvar_Set("skin", scratch);
	...
	{
		cmdparser.Cmd_AddCommand("menu_main", Menu.Menu_Main);
		cmdparser.Cmd_AddCommand("menu_game", Menu.Menu_Game);
		cmdparser.Cmd_AddCommand("menu_loadgame", Menu.Menu_LoadGame);
		cmdparser.Cmd_AddCommand("menu_savegame", Menu.Menu_SaveGame);
		cmdparser.Cmd_AddCommand("menu_joinserver", Menu.Menu_JoinServer);
		cmdparser.Cmd_AddCommand("menu_addressbook", Menu.Menu_AddressBook);
		cmdparser.Cmd_AddCommand("menu_startserver", Menu.Menu_StartServer);
		cmdparser.Cmd_AddCommand("menu_dmoptions", Menu.Menu_DMOptions);
		cmdparser.Cmd_AddCommand("menu_playerconfig", Menu.Menu_PlayerConfig);
		cmdparser.Cmd_AddCommand("menu_downloadoptions", Menu.Menu_DownloadOptions);
		cmdparser.Cmd_AddCommand("menu_credits", Menu.Menu_Credits);
		cmdparser.Cmd_AddCommand("menu_multiplayer", Menu.Menu_Multiplayer);
		cmdparser.Cmd_AddCommand("menu_video", Menu.Menu_Video);
		cmdparser.Cmd_AddCommand("menu_options", Menu.Menu_Options);
		cmdparser.Cmd_AddCommand("menu_keys", Menu.Menu_Keys);
		cmdparser.Cmd_AddCommand("menu_quit", Menu.Menu_Quit);
