...
public class CL_input
{
	...
	private static void KeyDown(kbutton_t b)
	{
		...
		{
			clientserver.Com_Printf("Three keys down for a button!\n");
	...
	{
		CL_input.in_impulse = Lib.atoi(cmdparser.Cmd_Argv(1));
	}
	...
	{
		cmdparser.Cmd_AddCommand("centerview", () => { IN.CenterView(); });
		cmdparser.Cmd_AddCommand("+moveup", () => { CL_input.IN_UpDown(); });
		cmdparser.Cmd_AddCommand("-moveup", () => { CL_input.IN_UpUp(); });
		cmdparser.Cmd_AddCommand("+movedown", () => { CL_input.IN_DownDown(); });
		cmdparser.Cmd_AddCommand("-movedown", () => { CL_input.IN_DownUp(); });
		cmdparser.Cmd_AddCommand("+left", () => { CL_input.IN_LeftDown(); });
		cmdparser.Cmd_AddCommand("-left", () => { CL_input.IN_LeftUp(); });
		cmdparser.Cmd_AddCommand("+right", () => { CL_input.IN_RightDown(); });
		cmdparser.Cmd_AddCommand("-right", () => { CL_input.IN_RightUp(); });
		cmdparser.Cmd_AddCommand("+forward", () => { CL_input.IN_ForwardDown(); });
		cmdparser.Cmd_AddCommand("-forward", () => { CL_input.IN_ForwardUp(); });
		cmdparser.Cmd_AddCommand("+back", () => { CL_input.IN_BackDown(); });
		cmdparser.Cmd_AddCommand("-back", () => { CL_input.IN_BackUp(); });
		cmdparser.Cmd_AddCommand("+lookup", () => { CL_input.IN_LookupDown(); });
		cmdparser.Cmd_AddCommand("-lookup", () => { CL_input.IN_LookupUp(); });
		cmdparser.Cmd_AddCommand("+lookdown", () => { CL_input.IN_LookdownDown(); });
		cmdparser.Cmd_AddCommand("-lookdown", () => { CL_input.IN_LookdownUp(); });
		cmdparser.Cmd_AddCommand("+strafe", () => { CL_input.IN_StrafeDown(); });
		cmdparser.Cmd_AddCommand("-strafe", () => { CL_input.IN_StrafeUp(); });
		cmdparser.Cmd_AddCommand("+moveleft", () => { CL_input.IN_MoveleftDown(); });
		cmdparser.Cmd_AddCommand("-moveleft", () => { CL_input.IN_MoveleftUp(); });
		cmdparser.Cmd_AddCommand("+moveright", () => { CL_input.IN_MoverightDown(); });
		cmdparser.Cmd_AddCommand("-moveright", () => { CL_input.IN_MoverightUp(); });
		cmdparser.Cmd_AddCommand("+speed", () => { CL_input.IN_SpeedDown(); });
		cmdparser.Cmd_AddCommand("-speed", () => { CL_input.IN_SpeedUp(); });
		cmdparser.Cmd_AddCommand("+attack", () => { CL_input.IN_AttackDown(); });
		cmdparser.Cmd_AddCommand("-attack", () => { CL_input.IN_AttackUp(); });
		cmdparser.Cmd_AddCommand("+use", () => { CL_input.IN_UseDown(); });
		cmdparser.Cmd_AddCommand("-use", () => { CL_input.IN_UseUp(); });
		cmdparser.Cmd_AddCommand("impulse", () => { CL_input.IN_Impulse(); });
		cmdparser.Cmd_AddCommand("+klook", () => { CL_input.IN_KLookDown(); });
		cmdparser.Cmd_AddCommand("-klook", () => { CL_input.IN_KLookUp(); });
		CL_input.cl_nodelta = cvar.Cvar_Get("cl_nodelta", "0", 0);
	}
	...
	public static void SendCmd()
	{
		...
		{
			if (Globals.cls.netchan.message.cursize != 0 || frame.curtime - Globals.cls.netchan.last_sent > 1000)
		...
		if (Globals.userinfo_modified)
		{
			...
			MSG.WriteString(Globals.cls.netchan.message, cvar.Cvar_Userinfo());
		}
