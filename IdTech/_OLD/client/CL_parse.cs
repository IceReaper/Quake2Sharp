...
public class CL_parse
{
	...
	{
		return filesystem.FS_Gamedir() + "/" + fn;
	}
	...
	public static bool CheckOrDownloadFile(string filename)
	{
		...
		{
			clientserver.Com_Printf("Refusing to download a path with ..\n");

			return true;
		}

		if (filesystem.FS_FileLength(filename) > 0)
		{
		...
		if (fp != null)
		{
			...
			clientserver.Com_Printf("Resuming " + Globals.cls.downloadname + "\n");
			MSG.WriteByte(Globals.cls.netchan.message, Defines.clc_stringcmd);
			MSG.WriteString(Globals.cls.netchan.message, "download " + Globals.cls.downloadname + " " + len);
		}
		else
		{
			clientserver.Com_Printf("Downloading " + Globals.cls.downloadname + "\n");
			MSG.WriteByte(Globals.cls.netchan.message, Defines.clc_stringcmd);
	...
	public static Action Download_f = () =>
	{
		...
		{
			clientserver.Com_Printf("Usage: download <filename>\n");

			return;
		}

		filename = cmdparser.Cmd_Argv(1);
		...
		if (filesystem.FS_FileLength(filename) > 0)
		{
			...
			// download
			clientserver.Com_Printf("File already exists.\n");
	...
	public static void ParseDownload()
	{
		...
		if (size == -1)
		{
			clientserver.Com_Printf("Server does not have this file.\n");
		...
		if (Globals.cls.download == null)
		{
			...
			if (Globals.cls.download == null)
			{
				...
				clientserver.Com_Printf("Failed to open " + Globals.cls.downloadtempname + "\n");
				Cl.RequestNextDownload();
	...
	{
		clientserver.Com_DPrintf("ParseServerData():Serverdata packet received.\n");
		...
		if (clientserver.Com_ServerState() != 0 && Defines.PROTOCOL_VERSION == 34)
		{
		}
		else if (i != Defines.PROTOCOL_VERSION)
			clientserver.Com_Error(Defines.ERR_DROP, "Server returned version " + i + ", not " + Defines.PROTOCOL_VERSION);
		...
		// set gamedir
		if ((str.Length > 0 && (filesystem.fs_gamedirvar.@string == null || filesystem.fs_gamedirvar.@string.Length == 0 || filesystem.fs_gamedirvar.@string.Equals(str)))
		    || (str.Length == 0 && (filesystem.fs_gamedirvar.@string != null || filesystem.fs_gamedirvar.@string.Length == 0)))
			cvar.Cvar_Set("game", str);
		...
		else
		{
			...
			//			clientserver.Com_Printf('\02' + str + "\n");
			clientserver.Com_Printf("Levelname:" + str + "\n");
	...
	public static void LoadClientinfo(clientinfo_t ci, string s)
	{
		...
		else
		{
			...
			{
				pos = 0;
				clientserver.Com_Error(Defines.ERR_FATAL, "Invalid model name:" + s);
			}
	...
	public static void ParseConfigString()
	{
		...
		if (i < 0 || i >= Defines.MAX_CONFIGSTRINGS)
			clientserver.Com_Error(Defines.ERR_DROP, "configstring > MAX_CONFIGSTRINGS");
	...
	public static void ParseStartSoundPacket()
	{
		...
		if ((flags & Defines.SND_ENT) != 0)
		{
			...
			if (ent > Defines.MAX_EDICTS)
				clientserver.Com_Error(Defines.ERR_DROP, "CL_ParseStartSoundPacket: ent = " + ent);
	...
	public static void SHOWNET(string s)
	{
		if (Globals.cl_shownet.value >= 2)
			clientserver.Com_Printf(Globals.net_message.readcount - 1 + ":" + s + "\n");
	}
	...
	public static void ParseServerMessage()
	{
		...
		//clientserver.Com_Printf(net_message.cursize + " ");
		...
		while (true)
		{
			...
			{
				clientserver.Com_Error(Defines.ERR_FATAL, "CL_ParseServerMessage: Bad server message:");
			...
			if (Globals.cl_shownet.value >= 2)
			{
				if (null == CL_parse.svc_strings[cmd])
					clientserver.Com_Printf(Globals.net_message.readcount - 1 + ":BAD CMD " + cmd + "\n");
				else
			...
			{
				default:
					clientserver.Com_Error(Defines.ERR_DROP, "CL_ParseServerMessage: Illegible server message\n");

					break;

				case Defines.svc_nop:
					//				clientserver.Com_Printf ("svc_nop\n");
					break;

				case Defines.svc_disconnect:
					clientserver.Com_Error(Defines.ERR_DISCONNECT, "Server disconnected\n");

					break;

				case Defines.svc_reconnect:
					clientserver.Com_Printf("Server disconnected, reconnecting\n");
				...
				case Defines.svc_print:
					...
					clientserver.Com_Printf(MSG.ReadString(Globals.net_message));
				...
				case Defines.svc_stufftext:
					var s = MSG.ReadString(Globals.net_message);
					clientserver.Com_DPrintf("stufftext: " + s + "\n");
					cmdparser.Cbuf_AddText(s);

					break;

				case Defines.svc_serverdata:
					cmdparser.Cbuf_Execute(); // make sure any stuffed commands are done
				...
				case Defines.svc_deltapacketentities:
					clientserver.Com_Error(Defines.ERR_DROP, "Out of place frame data");
