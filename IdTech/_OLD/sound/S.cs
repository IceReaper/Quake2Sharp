...
public class S
{
	...
	static S()
	{
		...
		{
			clientserver.Com_DPrintf("could not init dummy sound driver class.");
		}
	}
	...
	public static void Init()
	{
		...
		var cv = cvar.Cvar_Get("s_initsound", "1", 0);

		if (cv.value == 0.0f)
		{
			clientserver.Com_Printf("not initializing.\n");
			S.useDriver("dummy");
		...
		S.s_impl = cvar.Cvar_Get("s_impl", defaultDriver, Defines.CVAR_ARCHIVE);
		S.useDriver(S.s_impl.@string);

		if (S.impl.Init())
		{
			// driver ok
			cvar.Cvar_Set("s_impl", S.impl.getName());
		}
		else
		{
			// fallback
