...
public class WaveLoader
{
	...
	public static sfxcache_t LoadSound(sfx_t s)
	{
		...
		var data = filesystem.FS_LoadFile(namebuffer);

		if (data == null)
		{
			clientserver.Com_DPrintf("Couldn't load " + namebuffer + "\n");
		...
		if (info.channels != 1)
		{
			clientserver.Com_Printf(s.name + " is a stereo sample - ignoring\n");
		...
		{
			clientserver.Com_Printf(s.name + " is too long: " + len + " bytes?! ignoring.\n");
	...
	*/
	private static wavinfo_t GetWavinfo(string name, byte[] wav, int wavlength)
	{
		...
		if (!s.Equals("WAVE"))
		{
			clientserver.Com_Printf("Missing RIFF/WAVE chunks\n");
		...
		{
			clientserver.Com_Printf("Missing fmt chunk\n");
		...
		if (format != 1)
		{
			clientserver.Com_Printf("Microsoft PCM format only\n");
		...
		if (WaveLoader.data_p == 0)
		{
			clientserver.Com_Printf("Missing data chunk\n");
		...
		if (info.samples != 0)
		{
			if (samples < info.samples)
				clientserver.Com_Error(Defines.ERR_DROP, "Sound " + name + " has a bad loop length");
		}
		else
		{
