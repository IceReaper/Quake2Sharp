...
public class PMove
{
	...
	public class pml_t
	{
		...
		public float[] forward = { 0, 0, 0 }, right = { 0, 0, 0 }, up = { 0, 0, 0 };
	...
	private static readonly float[][] planes = new float[SV.MAX_CLIP_PLANES][];

	static PMove()
	{
		for (var n = 0; n < PMove.planes.Length; n++)
			PMove.planes[n] = new float[3];
	}
	...
	public static void PM_StepSlideMove_()
	{
		...
		for (bumpcount = 0; bumpcount < numbumps; bumpcount++)
		{
			...
			trace = PMove.pm.trace(PMove.pml.origin, PMove.pm.mins, PMove.pm.maxs, end);
			...
			Math3D.VectorCopy(trace.plane.normal, PMove.planes[numplanes]);
			numplanes++;

			// modify original_velocity so it parallels all of the clip planes
			for (i = 0; i < numplanes; i++)
			{
				PMove.PM_ClipVelocity(PMove.pml.velocity, PMove.planes[i], PMove.pml.velocity, 1.01f);

				for (j = 0; j < numplanes; j++)
				{
					if (j != i)
					{
						if (Math3D.DotProduct(PMove.pml.velocity, PMove.planes[j]) < 0)
							break; // not ok
					}
				}
			...
			else
			{
				...
				Math3D.CrossProduct(PMove.planes[0], PMove.planes[1], dir);
	...
	public static void PM_StepSlideMove()
	{
		...
		trace = PMove.pm.trace(up, PMove.pm.mins, PMove.pm.maxs, up);
	...
	public static void PM_WaterMove()
	{
		...
		if (0 == PMove.pm.cmd.forwardmove && 0 == PMove.pm.cmd.sidemove && 0 == PMove.pm.cmd.upmove)
	...
	public static void PM_AirMove()
	{
		...
		maxspeed = (PMove.pm.s.pm_flags & pmove_t.PMF_DUCKED) != 0 ? PMove.pm_duckspeed : PMove.pm_maxspeed;
	...
	public static void PM_CatagorizePosition()
	{
		...
		if (PMove.pml.velocity[2] > 180) //!!ZOID changed from 100 to 180 (ramp
			// accel)
		{
			PMove.pm.s.pm_flags &= (byte)~pmove_t.PMF_ON_GROUND;
		...
		else
		{
			...
			else
			{
				...
				{
					PMove.pm.s.pm_flags &= (byte)~(pmove_t.PMF_TIME_WATERJUMP | pmove_t.PMF_TIME_LAND | pmove_t.PMF_TIME_TELEPORT);
					PMove.pm.s.pm_time = 0;
				}

				if (0 == (PMove.pm.s.pm_flags & pmove_t.PMF_ON_GROUND))
				{
					// just hit the ground
					PMove.pm.s.pm_flags |= (byte)pmove_t.PMF_ON_GROUND;
					...
					{
						PMove.pm.s.pm_flags |= (byte)pmove_t.PMF_TIME_LAND;
	...
	public static void PM_CheckJump()
	{
		...
		{
			// not holding jump
			PMove.pm.s.pm_flags &= (byte)~pmove_t.PMF_JUMP_HELD;
	...
	public static void PM_CheckSpecialMovement()
	{
		...
		trace = PMove.pm.trace(PMove.pml.origin, PMove.pm.mins, PMove.pm.maxs, spot);
		...
		PMove.pm.s.pm_flags |= (byte)pmove_t.PMF_TIME_WATERJUMP;
	...
	public static void PM_CheckDuck()
	{
		...
		if (PMove.pm.s.pm_type == Defines.PM_DEAD)
			PMove.pm.s.pm_flags |= (byte)pmove_t.PMF_DUCKED;
		...
		else
		{
			...
			if ((PMove.pm.s.pm_flags & pmove_t.PMF_DUCKED) != 0)
			{
				// try to stand up
				PMove.pm.maxs[2] = 32;
				trace = PMove.pm.trace(PMove.pml.origin, PMove.pm.mins, PMove.pm.maxs, PMove.pml.origin);
	...
	public static bool PM_GoodPosition()
	{
		trace_t trace;
	...
	public static void PM_SnapPosition()
	{
		...
		for (j = 0; j < 8; j++)
		{
			bits = PMove.jitterbits[j];
			Math3D.VectorCopy(@base, PMove.pm.s.origin);

			for (i = 0; i < 3; i++)
			{
				if ((bits & (1 << i)) != 0)
					PMove.pm.s.origin[i] += (short)sign[i];
			}
		...
		// clientserver.Com_DPrintf("using previous_origin\n");
	}
	...
	public static void PM_InitialSnapPosition()
	{
		...
		for (z = 0; z < 3; z++)
		{
			PMove.pm.s.origin[2] = (short)(@base[2] + PMove.offset[z]);

			for (y = 0; y < 3; y++)
			{
				PMove.pm.s.origin[1] = (short)(@base[1] + PMove.offset[y]);

				for (x = 0; x < 3; x++)
				{
					PMove.pm.s.origin[0] = (short)(@base[0] + PMove.offset[x]);
	...
	public static void Pmove(pmove_t pmove)
	{
		...
		if (PMove.pm.s.pm_time != 0)
		{
			...
			msec = (int)((uint)PMove.pm.cmd.msec >> 3);
