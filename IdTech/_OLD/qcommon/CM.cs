...
public class CM
{
	...
	{
		clientserver.Com_DPrintf("CM_LoadMap(" + name + ")...\n");
		byte[] buf;
		dheader_t header;
		int length;

		CM.map_noareas = cvar.Cvar_Get("map_noareas", "0", 0);

		if (CM.map_name.Equals(name) && (clientload || 0 == cvar.Cvar_VariableValue("flushmap")))
		{
		...
		// load the file
		//
		buf = filesystem.FS_LoadFile(name);

		if (buf == null)
			clientserver.Com_Error(Defines.ERR_DROP, "Couldn't load " + name);
	...
	{
		clientserver.Com_DPrintf("CMod_LoadSubmodels()\n");
		dmodel_t @in;
		cmodel_t @out;
		int i, j, count;

		if (l.filelen % dmodel_t.SIZE != 0)
			clientserver.Com_Error(Defines.ERR_DROP, "CMod_LoadBmodel: funny lump size");

		count = l.filelen / dmodel_t.SIZE;

		if (count < 1)
			clientserver.Com_Error(Defines.ERR_DROP, "Map with no models");

		if (count > Defines.MAX_MAP_MODELS)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too many models");

		clientserver.Com_DPrintf(" numcmodels=" + count + "\n");
		CM.numcmodels = count;

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("submodles(headnode, <origin>, <mins>, <maxs>)\n");
	...
	{
		clientserver.Com_DPrintf("CMod_LoadSurfaces()\n");
		texinfo_t @in;
		mapsurface_t @out;
		int i, count;

		if (l.filelen % texinfo_t.SIZE != 0)
			clientserver.Com_Error(Defines.ERR_DROP, "MOD_LoadBmodel: funny lump size");

		count = l.filelen / texinfo_t.SIZE;

		if (count < 1)
			clientserver.Com_Error(Defines.ERR_DROP, "Map with no surfaces");

		if (count > Defines.MAX_MAP_TEXINFO)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too many surfaces");

		CM.numtexinfo = count;
		clientserver.Com_DPrintf(" numtexinfo=" + count + "\n");

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("surfaces:\n");
		...
		for (i = 0; i < count; i++)
		{
			...
			if (CM.debugloadmap)
				clientserver.Com_DPrintf($"|{@out.c.name,20}|{@out.rname,20}|{@out.c.value,6}|{@out.c.flags,6}|\n");
		}
	}
	...
	{
		clientserver.Com_DPrintf("CMod_LoadNodes()\n");
		dnode_t @in;
		int child;
		cnode_t @out;
		int i, j, count;
		...
		if (l.filelen % dnode_t.SIZE != 0)
			clientserver.Com_Error(Defines.ERR_DROP, "MOD_LoadBmodel: funny lump size:" + l.fileofs + "," + dnode_t.SIZE);

		count = l.filelen / dnode_t.SIZE;

		if (count < 1)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has no nodes");

		if (count > Defines.MAX_MAP_NODES)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too many nodes");

		CM.numnodes = count;
		clientserver.Com_DPrintf(" numnodes=" + count + "\n");

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("nodes(planenum, child[0], child[1])\n");
		...
		for (i = 0; i < count; i++)
		{
			...
			if (CM.debugloadmap)
				clientserver.Com_DPrintf($"|{@in.planenum,6}| {@out.children[0],6}| {@out.children[1],6}|\n");
		}
	}
	...
	{
		clientserver.Com_DPrintf("CMod_LoadBrushes()\n");
		dbrush_t @in;
		cbrush_t @out;
		int i, count;
		...
		if (count > Defines.MAX_MAP_BRUSHES)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too many brushes");

		CM.numbrushes = count;
		clientserver.Com_DPrintf(" numbrushes=" + count + "\n");

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("brushes:(firstbrushside, numsides, contents)\n");
		...
		for (i = 0; i < count; i++)
		{
			...
			if (CM.debugloadmap)
				clientserver.Com_DPrintf($"| {@out.firstbrushside,6}| {@out.numsides,6}| {@out.contents:X8}|\n");
		}
	}
	...
	{
		clientserver.Com_DPrintf("CMod_LoadLeafs()\n");
		int i;
		cleaf_t @out;
		dleaf_t @in;
		int count;
		...
		if (count < 1)
			clientserver.Com_Error(Defines.ERR_DROP, "Map with no leafs");

		// need to save space for box planes
		if (count > Defines.MAX_MAP_PLANES)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too many planes");

		clientserver.Com_DPrintf(" numleafes=" + count + "\n");

		CM.numleafs = count;
		CM.numclusters = 0;

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("cleaf-list:(contents, cluster, area, firstleafbrush, numleafbrushes)\n");
		...
		clientserver.Com_DPrintf(" numclusters=" + CM.numclusters + "\n");

		if (CM.map_leafs[0].contents != Defines.CONTENTS_SOLID)
			clientserver.Com_Error(Defines.ERR_DROP, "Map leaf 0 is not CONTENTS_SOLID");
		...
		if (CM.emptyleaf == -1)
			clientserver.Com_Error(Defines.ERR_DROP, "Map does not have an empty leaf");
	}

	/** Loads planes. */
	public static void CMod_LoadPlanes(lump_t l)
	{
		clientserver.Com_DPrintf("CMod_LoadPlanes()\n");
		int i, j;
		cplane_t @out;
		dplane_t @in;
		int count;
		...
		if (count < 1)
			clientserver.Com_Error(Defines.ERR_DROP, "Map with no planes");

		// need to save space for box planes
		if (count > Defines.MAX_MAP_PLANES)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too many planes");

		clientserver.Com_DPrintf(" numplanes=" + count + "\n");

		CM.numplanes = count;

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("cplanes(normal[0],normal[1],normal[2], dist, type, signbits)\n");
	...
	{
		clientserver.Com_DPrintf("CMod_LoadLeafBrushes()\n");
		...
		if (count > Defines.MAX_MAP_LEAFBRUSHES)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too many leafbrushes");
		...
		if (CM.debugloadmap)
			clientserver.Com_DPrintf("map_brushes:\n");
		...
		for (var i = 0; i < count; i++)
		{
			...
			if (CM.debugloadmap)
				clientserver.Com_DPrintf($"|{i,6}|{@out[i],6}|\n");
		}
	}
	...
	{
		clientserver.Com_DPrintf("CMod_LoadBrushSides()\n");
		int i, j;
		cbrushside_t @out;
		dbrushside_t @in;
		int count;
		...
		clientserver.Com_DPrintf(" numbrushsides=" + count + "\n");

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("brushside(planenum, surfacenum):\n");
		...
		for (i = 0; i < count; i++)
		{
			...
			if (j >= CM.numtexinfo)
				clientserver.Com_Error(Defines.ERR_DROP, "Bad brushside texinfo");
			...
			if (CM.debugloadmap)
				clientserver.Com_DPrintf($"| {num,6}| {j,6}|\n");
		}
	}
	...
	{
		clientserver.Com_DPrintf("CMod_LoadAreas()\n");
		int i;
		carea_t @out;
		darea_t @in;
		int count;
		...
		if (count > Defines.MAX_MAP_AREAS)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too many areas");

		clientserver.Com_DPrintf(" numareas=" + count + "\n");
		CM.numareas = count;

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("areas(numportals, firstportal)\n");
		...
		for (i = 0; i < count; i++)
		{
			...
			if (CM.debugloadmap)
				clientserver.Com_DPrintf($"| {@out.numareaportals,6}| {@out.firstareaportal,6}|\n");
		}
	}
	...
	{
		clientserver.Com_DPrintf("CMod_LoadAreaPortals()\n");
		int i;
		dareaportal_t @out;
		dareaportal_t @in;
		int count;
		...
		clientserver.Com_DPrintf(" numareaportals=" + count + "\n");

		if (CM.debugloadmap)
			clientserver.Com_DPrintf("areaportals(portalnum, otherarea)\n");
		...
		for (i = 0; i < count; i++)
		{
			...
			if (CM.debugloadmap)
				clientserver.Com_DPrintf($"|{@out.portalnum,6}|{@out.otherarea,6}|\n");
		}
	}
	...
	{
		clientserver.Com_DPrintf("CMod_LoadVisibility()\n");

		CM.numvisibility = l.filelen;

		clientserver.Com_DPrintf(" numvisibility=" + CM.numvisibility + "\n");

		if (l.filelen > Defines.MAX_MAP_VISIBILITY)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too large visibility lump");
	...
	{
		clientserver.Com_DPrintf("CMod_LoadEntityString()\n");

		CM.numentitychars = l.filelen;

		if (l.filelen > Defines.MAX_MAP_ENTSTRING)
			clientserver.Com_Error(Defines.ERR_DROP, "Map has too large entity lump");
	...
	{
		if (name == null || name[0] != '*')
			clientserver.Com_Error(Defines.ERR_DROP, "CM_InlineModel: bad name");

		var num = Lib.atoi(name[1..]);

		if (num < 1 || num >= CM.numcmodels)
			clientserver.Com_Error(Defines.ERR_DROP, "CM_InlineModel: bad number");
	...
	public static int CM_LeafContents(int leafnum)
	{
		if (leafnum < 0 || leafnum >= CM.numleafs)
			clientserver.Com_Error(Defines.ERR_DROP, "CM_LeafContents: bad number");
	...
	{
		if (leafnum < 0 || leafnum >= CM.numleafs)
			clientserver.Com_Error(Defines.ERR_DROP, "CM_LeafCluster: bad number");
	...
	{
		if (leafnum < 0 || leafnum >= CM.numleafs)
			clientserver.Com_Error(Defines.ERR_DROP, "CM_LeafArea: bad number");
	...
	public static void CM_InitBoxHull()
	{
		...
		if (CM.numnodes + 6 > Defines.MAX_MAP_NODES
			...
			clientserver.Com_Error(Defines.ERR_DROP, "Not enough room for box tree");
	...
	private static void CM_BoxLeafnums_r(int nodenum)
	{
		...
		while (true)
		{
			if (nodenum < 0)
			{
				...
				{
					clientserver.Com_DPrintf("CM_BoxLeafnums_r: overflow\n");
	...
	public static void CM_DecompressVis(byte[] @in, int offset, byte[] @out)
	{
		...
		do
		{
			...
			{
				c = row - outp;
				clientserver.Com_DPrintf("warning: Vis decompression overrun\n");
			}
	...
	{
		//clientserver.Com_Printf("FloodArea_r(" + floodnum + ")...\n");
		int i;
		dareaportal_t p;
		...
		if (area.floodvalid == CM.floodvalid)
		{
			...
			clientserver.Com_Error(Defines.ERR_DROP, "FloodArea_r: reflooded");
		}
	...
	{
		clientserver.Com_DPrintf("FloodAreaConnections...\n");
	...
	public static void CM_SetAreaPortalState(int portalnum, bool open)
	{
		if (portalnum > CM.numareaportals)
			clientserver.Com_Error(Defines.ERR_DROP, "areaportal > numareaportals");
	...
	public static bool CM_AreasConnected(int area1, int area2)
	{
		...
		if (area1 > CM.numareas || area2 > CM.numareas)
			clientserver.Com_Error(Defines.ERR_DROP, "area > numareas");
	...
	public static void CM_WritePortalState(BinaryWriter os)
	{
		...
		{
			clientserver.Com_Printf("ERROR:" + e);
			Console.WriteLine(e);
		}
	}
