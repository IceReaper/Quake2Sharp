...
public class Netchan
{
	...
	public static void Netchan_Init()
	{
		...
		Netchan.qport = cvar.Cvar_Get("qport", "" + port, Defines.CVAR_NOSET);
	}

	private static readonly byte[] send_buf = new byte[Defines.MAX_MSGLEN];
	private static readonly sizebuf_t send = new();
	...
	public static void Netchan_OutOfBand(int net_socket, netadr_t adr, int length, byte[] data)
	{
		...
		SZ.Init(Netchan.send, Netchan.send_buf, Defines.MAX_MSGLEN);

		MSG.WriteInt(Netchan.send, -1); // -1 sequence means out of band
		SZ.Write(Netchan.send, data, length);
	...
	public static void Setup(int sock, netchan_t chan, netadr_t adr, int qport)
	{
		...
		chan.qport = qport;
		chan.last_received = frame.curtime;
	...
	public static void Transmit(netchan_t chan, int length, byte[] data)
	{
		...
		if (chan.message.overflowed)
		{
			...
			clientserver.Com_Printf(NET.AdrToString(chan.remote_address) + ":Outgoing message overflow\n");

			return;
		}

		send_reliable = Netchan.Netchan_NeedReliable(chan) ? 1 : 0;

		if (chan.reliable_length == 0 && chan.message.cursize != 0)
		{
			Array.Copy(chan.message_buf, 0, chan.reliable_buf, 0, chan.message.cursize);
		...
		// write the packet header
		SZ.Init(Netchan.send, Netchan.send_buf, Netchan.send_buf.Length);

		w1 = (chan.outgoing_sequence & ~(1 << 31)) | (send_reliable << 31);
		w2 = (chan.incoming_sequence & ~(1 << 31)) | (chan.incoming_reliable_sequence << 31);

		chan.outgoing_sequence++;
		chan.last_sent = (int)frame.curtime;

		MSG.WriteInt(Netchan.send, w1);
		MSG.WriteInt(Netchan.send, w2);

		// send the qport if we are a client
		if (chan.sock == Defines.NS_CLIENT)
			MSG.WriteShort(Netchan.send, (int)Netchan.qport.value);

		// copy the reliable message to the packet first
		if (send_reliable != 0)
		{
			SZ.Write(Netchan.send, chan.reliable_buf, chan.reliable_length);
		...
		if (Netchan.send.maxsize - Netchan.send.cursize >= length)
			...
			clientserver.Com_Printf("Netchan_Transmit: dumped unreliable\n");
	...
	public static bool Process(netchan_t chan, sizebuf_t msg)
	{
		...
		var sequence = (uint)MSG.ReadLong(msg);
		var sequence_ack = (uint)MSG.ReadLong(msg);
		...
		if (Netchan.showpackets.value != 0)
		{
			if (reliable_message != 0)
			{
				...
				clientserver.Com_Printf("recv " + msg.cursize + " : s=" + sequence + " ack=" + sequence_ack + " rack=" + reliable_ack + "\n");
		}
		...
		{
			if (Netchan.showdrop.value != 0)
				clientserver.Com_Printf(NET.AdrToString(chan.remote_address) + ":Out of order packet " + sequence + " at " + chan.incoming_sequence + "\n");
		...
		{
			if (Netchan.showdrop.value != 0)
				clientserver.Com_Printf(NET.AdrToString(chan.remote_address) + ":Dropped " + chan.dropped + " packets at " + sequence + "\n");
		}
		...
		//
		chan.last_received = (int)frame.curtime;
