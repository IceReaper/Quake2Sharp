...
public partial class OpenGLRenderApi
{
	...
	*/
	protected mleaf_t Mod_PointInLeaf(float[] p, model_t model)
	{
		...
		if (model == null || model.nodes == null)
			clientserver.Com_Error(Defines.ERR_DROP, "Mod_PointInLeaf: bad model");
	...
	*/
	private model_t Mod_ForName(string name, bool crash)
	{
		...
		if (name == null || name.Length == 0)
			clientserver.Com_Error(Defines.ERR_DROP, "Mod_ForName: NULL name");
		...
		if (name[0] == '*')
		{
			...
			if (i < 1 || this.r_worldmodel == null || i >= this.r_worldmodel.numsubmodels)
				clientserver.Com_Error(Defines.ERR_DROP, "bad inline model number");
		...
		if (i == this.mod_numknown)
		{
			if (this.mod_numknown == OpenGLRenderApi.MAX_MOD_KNOWN)
				clientserver.Com_Error(Defines.ERR_DROP, "mod_numknown == MAX_MOD_KNOWN");
		...
		// load the file
		//
		this.fileBuffer = filesystem.FS_LoadFile(name);

		if (this.fileBuffer == null)
		{
			if (crash)
				clientserver.Com_Error(Defines.ERR_DROP, "Mod_NumForName: " + mod.name + " not found");
		...
		switch (ident)
		{
			...
			default:
				clientserver.Com_Error(Defines.ERR_DROP, "Mod_NumForName: unknown fileid for " + mod.name);
	...
	*/
	private void Mod_LoadVertexes(lump_t l)
	{
		...
		if (l.filelen % mvertex_t.DISK_SIZE != 0)
			clientserver.Com_Error(Defines.ERR_DROP, "MOD_LoadBmodel: funny lump size in " + this.loadmodel.name);
	...
	private void Mod_LoadFaces(lump_t l)
	{
		...
		for (surfnum = 0; surfnum < count; surfnum++)
		{
			...
			if (ti < 0 || ti >= this.loadmodel.numtexinfo)
				clientserver.Com_Error(Defines.ERR_DROP, "MOD_LoadBmodel: bad texinfo number");
	...
	private void Mod_LoadMarksurfaces(lump_t l)
	{
		...
		for (i = 0; i < count; i++)
		{
			...
			if (j < 0 || j >= this.loadmodel.numsurfaces)
				clientserver.Com_Error(Defines.ERR_DROP, "Mod_ParseMarksurfaces: bad surface number");
	...
	*/
	private void Mod_LoadSurfedges(lump_t l)
	{
		...
		if (count < 1 || count >= Defines.MAX_MAP_SURFEDGES)
			clientserver.Com_Error(Defines.ERR_DROP, "MOD_LoadBmodel: bad surfedges count in " + this.loadmodel.name + ": " + count);
	...
	*/
	private void Mod_LoadBrushModel(model_t mod, BinaryReader buffer)
	{
		...
		if (this.loadmodel != this.mod_known[0])
			clientserver.Com_Error(Defines.ERR_DROP, "Loaded a brush model after the world");
		...
		if (i != Defines.BSPVERSION)
			clientserver.Com_Error(Defines.ERR_DROP, "Mod_LoadBrushModel: " + mod.name + " has wrong version number (" + i + " should be " + Defines.BSPVERSION + ")");
		...
		for (i = 0; i < mod.numsubmodels; i++)
		{
			...
			if (starmod.firstnode >= this.loadmodel.numnodes)
				clientserver.Com_Error(Defines.ERR_DROP, "Inline model " + i + " has bad firstnode");
	...
	*/
	private void Mod_LoadAliasModel(model_t mod, BinaryReader buffer)
	{
		...
		if (pheader.skinheight > Base.MAX_LBM_HEIGHT)
			clientserver.Com_Error(Defines.ERR_DROP, "model " + mod.name + " has a skin taller than " + Base.MAX_LBM_HEIGHT);

		if (pheader.num_xyz <= 0)
			clientserver.Com_Error(Defines.ERR_DROP, "model " + mod.name + " has no vertices");

		if (pheader.num_xyz > qfiles.MAX_VERTS)
			clientserver.Com_Error(Defines.ERR_DROP, "model " + mod.name + " has too many vertices");

		if (pheader.num_st <= 0)
			clientserver.Com_Error(Defines.ERR_DROP, "model " + mod.name + " has no st vertices");

		if (pheader.num_tris <= 0)
			clientserver.Com_Error(Defines.ERR_DROP, "model " + mod.name + " has no triangles");

		if (pheader.num_frames <= 0)
			clientserver.Com_Error(Defines.ERR_DROP, "model " + mod.name + " has no frames");
	...
	*/
	private void Mod_LoadSpriteModel(model_t mod, BinaryReader buffer)
	{
		...
		if (sprout.numframes > qfiles.MAX_MD2SKINS)
			clientserver.Com_Error(Defines.ERR_DROP, $"{mod.name} has too many frames ({sprout.numframes} > {qfiles.MAX_MD2SKINS})");
	...
	*/
	public override void R_BeginRegistration(string model)
	{
		...
		flushmap = cvar.Cvar_Get("flushmap", "0", 0);
