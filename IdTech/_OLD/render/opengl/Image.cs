...
public partial class OpenGLRenderApi
{
	...
	*/
	private byte[] LoadPCX(string filename, byte[][] palette, ref Size dim)
	{
		...
		// load the file
		//
		var raw = filesystem.FS_LoadFile(filename);
	...
	*/
	private byte[] LoadTGA(string name, ref Size dim)
	{
		...
		// load the file
		//
		raw = filesystem.FS_LoadFile(name);
		...
		if (targa_header.image_type != 2 && targa_header.image_type != 10)
			clientserver.Com_Error(Defines.ERR_DROP, "LoadTGA: Only type 2 and 10 targa RGB images supported\n");

		if (targa_header.colormap_type != 0 || (targa_header.pixel_size != 32 && targa_header.pixel_size != 24))
			clientserver.Com_Error(Defines.ERR_DROP, "LoadTGA: Only 32 or 24 bit images supported (no colormaps)\n");
	...
	*/
	...
	private bool GL_Upload32(int[] data, int width, int height, bool mipmap)
	{
		...
		if (scaled_width * scaled_height > 256 * 256)
			clientserver.Com_Error(Defines.ERR_DROP, "GL_Upload32: too big");
	...
	*/
	...
	private bool GL_Upload8(byte[] data, int width, int height, bool mipmap, bool is_sky)
	{
		...
		if (s > this.trans.Length)
			clientserver.Com_Error(Defines.ERR_DROP, "GL_Upload8: too large");
	...
	protected image_t GL_LoadPic(string name, byte[] pic, int width, int height, int type, int bits)
	{
		...
		if (i == this.numgltextures)
		{
			if (this.numgltextures == Base.MAX_GLTEXTURES)
				clientserver.Com_Error(Defines.ERR_DROP, "MAX_GLTEXTURES");
		...
		if (name.Length > Defines.MAX_QPATH)
			clientserver.Com_Error(Defines.ERR_DROP, "Draw_LoadPic: \"" + name + "\" is too long");
	...
	*/
	private image_t GL_LoadWal(string name)
	{
		...
		var raw = filesystem.FS_LoadFile(name);
	...
	*/
	protected image_t GL_FindImage(string name, int type)
	{
		...
		if (name == null || name.Length < 5)
			return null; //	clientserver.Com_Error (ERR_DROP, "GL_FindImage: NULL name");

		//	clientserver.Com_Error (ERR_DROP, "GL_FindImage: bad name: %s", name);
	...
	*/
	protected void Draw_GetPalette()
	{
		...
		if (palette[0] == null || palette[0].Length != 768)
			clientserver.Com_Error(Defines.ERR_FATAL, "Couldn't load pics/colormap.pcx");
	...
	*/
	protected void GL_InitImages()
	{
		...
		this.intensity = cvar.Cvar_Get("gl1_intensity", "2", 0);

		if (this.intensity.value <= 1)
			cvar.Cvar_Set("gl1_intensity", "1");
		...
		{
			this.gl_state.d_16to8table = filesystem.FS_LoadFile("pics/16to8.dat");

			if (this.gl_state.d_16to8table == null)
				clientserver.Com_Error(Defines.ERR_FATAL, "Couldn't load pics/16to8.pcx");
		}
