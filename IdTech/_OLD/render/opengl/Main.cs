...
using Quake2Sharp.render.opengl.types;
...
public partial class OpenGLRenderApi : RenderAPI
{
		...
		for (i = 0; i < this.r_newrefdef.num_entities; i++)
		{
			...
			else
			{
				...
				switch (this.currentmodel.type)
				{
					...
					default:
						clientserver.Com_Error(Defines.ERR_DROP, "Bad modeltype");
	...
	*/
	private void R_RenderView(refdef_t fd)
	{
		...
		if (this.r_newrefdef == null)
			clientserver.Com_Error(Defines.ERR_DROP, "R_RenderView: refdef_t fd is null");

		if (this.r_worldmodel == null && (this.r_newrefdef.rdflags & Defines.RDF_NOWORLDMODEL) == 0)
			clientserver.Com_Error(Defines.ERR_DROP, "R_RenderView: NULL worldmodel");
	...
	*/
	...
	{
		this.r_lefthand = cvar.Cvar_Get("hand", "0", Defines.CVAR_USERINFO | Defines.CVAR_ARCHIVE);
		this.r_norefresh = cvar.Cvar_Get("r_norefresh", "0", 0);
		this.r_fullbright = cvar.Cvar_Get("r_fullbright", "0", 0);
		this.r_drawentities = cvar.Cvar_Get("r_drawentities", "1", 0);
		this.r_drawworld = cvar.Cvar_Get("r_drawworld", "1", 0);
		this.r_novis = cvar.Cvar_Get("r_novis", "0", 0);
		this.r_nocull = cvar.Cvar_Get("r_nocull", "0", 0);
		this.r_lerpmodels = cvar.Cvar_Get("r_lerpmodels", "1", 0);
		this.r_speeds = cvar.Cvar_Get("r_speeds", "0", 0);

		this.r_lightlevel = cvar.Cvar_Get("r_lightlevel", "1", 0);

		this.gl_nosubimage = cvar.Cvar_Get("gl_nosubimage", "0", 0);
		this.gl_allow_software = cvar.Cvar_Get("gl_allow_software", "0", 0);

		this.gl_particle_min_size = cvar.Cvar_Get("gl1_particle_min_size", "2", Defines.CVAR_ARCHIVE);
		this.gl_particle_max_size = cvar.Cvar_Get("gl1_particle_max_size", "40", Defines.CVAR_ARCHIVE);
		this.gl_particle_size = cvar.Cvar_Get("gl1_particle_size", "40", Defines.CVAR_ARCHIVE);
		this.gl_particle_att_a = cvar.Cvar_Get("gl1_particle_att_a", "0.01", Defines.CVAR_ARCHIVE);
		this.gl_particle_att_b = cvar.Cvar_Get("gl1_particle_att_b", "0.0", Defines.CVAR_ARCHIVE);
		this.gl_particle_att_c = cvar.Cvar_Get("gl1_particle_att_c", "0.01", Defines.CVAR_ARCHIVE);

		this.gl_modulate = cvar.Cvar_Get("r_modulate", "1.5", Defines.CVAR_ARCHIVE);
		this.gl_log = cvar.Cvar_Get("gl_log", "0", 0);
		this.gl_bitdepth = cvar.Cvar_Get("gl_bitdepth", "0", 0);
		this.gl_mode = cvar.Cvar_Get("r_mode", "1", Defines.CVAR_ARCHIVE);
		this.gl_lightmap = cvar.Cvar_Get("r_lighmap", "0", 0);
		this.gl_shadows = cvar.Cvar_Get("r_shadows", "0", Defines.CVAR_ARCHIVE);
		this.gl_dynamic = cvar.Cvar_Get("gl1_dynamic", "1", 0);
		this.gl_nobind = cvar.Cvar_Get("gl_nobind", "0", 0);
		this.gl_round_down = cvar.Cvar_Get("gl1_round_down", "1", 0);
		this.gl_picmip = cvar.Cvar_Get("gl1_picmip", "0", 0);
		this.gl_skymip = cvar.Cvar_Get("gl_skymip", "0", 0);
		this.gl_showtris = cvar.Cvar_Get("gl_showtris", "0", 0);
		this.gl_ztrick = cvar.Cvar_Get("gl1_ztrick", "0", 0);
		this.gl_finish = cvar.Cvar_Get("gl_finish", "0", Defines.CVAR_ARCHIVE);
		this.gl_clear = cvar.Cvar_Get("r_clear", "0", 0);
		this.gl_cull = cvar.Cvar_Get("gl_cull", "1", 0);
		this.gl_polyblend = cvar.Cvar_Get("gl1_polyblend", "1", 0);
		this.gl_flashblend = cvar.Cvar_Get("gl1_flashblend", "0", 0);
		this.gl_playermip = cvar.Cvar_Get("gl_playermip", "0", 0);
		this.gl_monolightmap = cvar.Cvar_Get("gl_monolightmap", "0", 0);
		this.gl_driver = cvar.Cvar_Get("gl_driver", "opengl32", Defines.CVAR_ARCHIVE);
		this.gl_texturemode = cvar.Cvar_Get("gl_texturemode", "GL_NEAREST_MIPMAP_NEAREST", Defines.CVAR_ARCHIVE);
		this.gl_texturealphamode = cvar.Cvar_Get("gl1_texturealphamode", "default", Defines.CVAR_ARCHIVE);
		this.gl_texturesolidmode = cvar.Cvar_Get("gl1_texturesolidmode", "default", Defines.CVAR_ARCHIVE);
		this.gl_lockpvs = cvar.Cvar_Get("r_lockpvs", "0", 0);

		this.gl_vertex_arrays = cvar.Cvar_Get("gl_vertex_arrays", "0", Defines.CVAR_ARCHIVE);

		this.gl_ext_swapinterval = cvar.Cvar_Get("gl_ext_swapinterval", "1", Defines.CVAR_ARCHIVE);
		this.gl_ext_palettedtexture = cvar.Cvar_Get("gl_ext_palettedtexture", "0", Defines.CVAR_ARCHIVE);
		this.gl_ext_multitexture = cvar.Cvar_Get("gl_ext_multitexture", "1", Defines.CVAR_ARCHIVE);
		this.gl_ext_pointparameters = cvar.Cvar_Get("gl_ext_pointparameters", "1", Defines.CVAR_ARCHIVE);
		this.gl_ext_compiled_vertex_array = cvar.Cvar_Get("gl_ext_compiled_vertex_array", "1", Defines.CVAR_ARCHIVE);

		this.gl_drawbuffer = cvar.Cvar_Get("gl_drawbuffer", "GL_BACK", 0);
		this.gl_swapinterval = cvar.Cvar_Get("r_vsync", "1", Defines.CVAR_ARCHIVE);

		this.gl_saturatelighting = cvar.Cvar_Get("gl_saturatelighting", "0", 0);

		this.gl_3dlabs_broken = cvar.Cvar_Get("gl_3dlabs_broken", "1", Defines.CVAR_ARCHIVE);

		Base.vid_fullscreen = cvar.Cvar_Get("vid_fullscreen", "0", Defines.CVAR_ARCHIVE);
		this.vid_gamma = cvar.Cvar_Get("vid_gamma", "1.0", Defines.CVAR_ARCHIVE);
		this.vid_ref = cvar.Cvar_Get("vid_ref", "Dummy", Defines.CVAR_ARCHIVE);

		cmdparser.Cmd_AddCommand("imagelist", () => { this.GL_ImageList_f(); });

		cmdparser.Cmd_AddCommand("screenshot", () => { Base.glImpl.screenshot(); });

		cmdparser.Cmd_AddCommand("modellist", () => { this.Mod_Modellist_f(); });

		cmdparser.Cmd_AddCommand("gl_strings", () => { this.GL_Strings_f(); });
	}
	...
	*/
	protected bool R_SetMode()
	{
		...
		//			cvar.Cvar_SetValue("vid_fullscreen", (vid_fullscreen.value > 0.0f) ? 0.0f : 1.0f);
		...
		else
		{
			...
			{
				cvar.Cvar_SetValue("vid_fullscreen", 0);
				Base.vid_fullscreen.modified = false;
			...
			{
				cvar.Cvar_SetValue("r_mode", this.gl_state.prev_mode);
				this.gl_mode.modified = false;
	...
	public override bool R_Init2()
	{
		...
		*/
		...
		if ((this.gl_config.renderer & Base.GL_RENDERER_POWERVR) != 0)
			cvar.Cvar_Set("scr_drawall", "1");
		else
			cvar.Cvar_Set("scr_drawall", "0");

		// #ifdef __linux__
		cvar.Cvar_SetValue("gl_finish", 1);
		...
		*/
		...
		if (this.gl_config.extensions_string.IndexOf("GL_ARB_multitexture") >= 0)
		{
			if (this.gl_ext_multitexture.value != 0.0f)
			{
				...
				cvar.Cvar_SetValue("r_fullbright", 1);
			}
			else
			{
				VID.Printf(Defines.PRINT_ALL, "...ignoring GL_ARB_multitexture\n");
				cvar.Cvar_SetValue("r_fullbright", 0);
			}
		}
		else
		{
	...
	*/
	...
	{
		cmdparser.Cmd_RemoveCommand("modellist");
		cmdparser.Cmd_RemoveCommand("screenshot");
		cmdparser.Cmd_RemoveCommand("imagelist");
		cmdparser.Cmd_RemoveCommand("gl_strings");
